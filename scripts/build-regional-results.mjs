...
/*
  ★★★**岐阜だけは切り替えなかった**（2026-09-02 その2。**作ったが `ADAPTERS` に入れていない**）。

  ★**HSB flash の岐阜は、夏の紙が1枚も読めない**（「スロット番号の列が 0 本（2本のはず）」）——
  **第101〜108回のどれも組み立てられず、夏に残るのは2020年の代替大会だけになる。**
  ★**連盟からは夏が5大会（321試合）取れている**ので、**切り替えると選手権が丸ごと消える。**
  ★★**試合数だけ見ると 344 → 448 で増えるので、数だけでは気づけない** ——
  **季節ごとの大会数**（春1夏5秋0 → 春6夏1秋5）を見て分かった。
  ★**切り替えの判断は「季節ごとに減っていないか」まで見ること。**

  ★**春季・秋季は読めている**ので、**夏の紙が読めるようになれば切り替えられる**
  （そのときは連盟の春季1大会と二重にならないか確かめること）。
*/
