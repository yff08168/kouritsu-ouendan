...
function writeDistrict(district) {
  ...
  */
  ...
  /*
    ★★★**書き出した内容を「前の生成物」として持ち直す**（2026-09-06。**実際にデータを失っていた**）。

    **1つの県に2つのアダプタがある**（主＝連盟、副＝HSB flash の穴埋め。24県）。
    `previousDistrict` は**最初に読んだときの中身を憶えたまま**なので、
    **副が走るころには、主が書いた新しい内容ではなく、実行前のファイルを返していた。**

    副が0件の季節（**「主が持っている年×季節には入らない」ので普通に起きる**）は
    「前の内容を残す」に落ちる —— **そこで返るのが実行前の中身**なので、
    **主がその回に取った新しい試合が、そのまま消える。**

        神奈川の秋季  主が 745試合（9月5日の24試合を含む）を書く
                      → 副が 721試合（前回のファイル）で上書き
                      → **昨日の試合が生成物から消える**

    ★**警告は出る**（「前は N 試合あったのに…」）が、**主が取れているので異常に見えない。**
    ★★**気づけたのは、同じ県のログに書き出しが2回並び、試合数が減っていたから。**
    ★**副のアダプタは主のうしろに置く決まり**なので、必ずこの向きで壊れる。
  */
  previousCache.set(d.slug, JSON.parse(file));
