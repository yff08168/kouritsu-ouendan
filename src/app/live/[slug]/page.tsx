 ...
 */
/*
  ★★★★**このページは必ずキャッシュに載せること**（2026-09-10。**サイトが丸ごと止まった**）。

  ------------------------------------------------------------------
  ★★★ 2026-09-08 に `force-dynamic` にして、翌日サイトが落ちた

  **盤が遅れる**（運営者から「法政二の試合がこのページでは0-0だけど、遷移すると7-0」）のを
  直すために**毎回作り直す**形にした。**遅れは直ったが、費用が跳ねた。**

  ★★★**このページには `LiveRefresh` があり、開いているタブが60秒ごとに
  `router.refresh()` を呼ぶ。**
  **キャッシュに載っていれば CDN が返すので、サーバーは動かない。**
  **`force-dynamic` だと、その60秒ごとの1回1回がサーバーの実行になる** ——
  **開いているタブ1つにつき1時間で60回**、しかも**学校マスタの索引まで引く。**

  ★★**費用が「時間」ではなく「見ている人の数」で決まる形にしてしまった。**
  秋の大会が始まって人が増えた日に、**Vercel（`DEPLOYMENT_DISABLED`）と
  Supabase（`exceed_egress_quota`）の両方の枠を使い切り、全ページが402になった。**

  ------------------------------------------------------------------
  ★★★**決めごと: 自動更新のあるページを `force-dynamic` にしないこと。**

  **ISR なら、何人が見ていても焼き直しは15秒に1回。** 費用は時間で頭打ちになる。
  ★**遅れは 15秒（ページ）＋ 60秒（出典の取得）が上限**で、
  **`force-dynamic` の60秒と比べて実用上ほとんど変わらない** ——
  **自動更新でページが絶えず叩かれるので、期限切れのまま放置される時間が短い。**
  ★**2026-09-08 に測った「10分古い」は、誰も見ていない時間帯に期限が切れていたから。**
  ★**空の `generateStaticParams` が要る** —— これが無いとこの版の Next は
  動的区間を毎回作り直す（`revalidate` を書いても効かない）。
*/
