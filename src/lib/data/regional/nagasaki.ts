// このファイルは scripts/build-regional-results.mjs が生成する。直接編集しない。
// 出典: 長崎県高等学校野球連盟（https://nagasaki-kouyaren.com/）

import type { RegionalDistrict } from "@/lib/regional-results";

export const REGIONAL_NAGASAKI: RegionalDistrict = {
  "slug": "nagasaki",
  "district": "長崎",
  "sourceName": "長崎県高等学校野球連盟",
  "sourceUrl": "https://nagasaki-kouyaren.com/",
  "games": [
    {
      "date": "2026-03-20",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎北",
          "score": 2,
          "won": false,
          "name": "長崎北高校",
          "slug": "nagasakikita"
        },
        {
          "display": "鹿町工",
          "score": 6,
          "won": true,
          "name": "鹿町工業高校",
          "slug": "shikamachikogyo"
        }
      ]
    },
    {
      "date": "2026-03-20",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎工",
          "score": 6,
          "won": true,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        },
        {
          "display": "佐世保北",
          "score": 5,
          "won": false,
          "name": "佐世保北高校",
          "slug": "sasebokita"
        }
      ]
    },
    {
      "date": "2026-03-20",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "波佐見",
          "score": 7,
          "won": true,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "小浜",
          "score": 1,
          "won": false,
          "name": "小浜高校",
          "slug": "obama"
        }
      ]
    },
    {
      "date": "2026-03-20",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "佐世保高専",
          "score": 1,
          "won": true,
          "name": "佐世保工業高専",
          "slug": "sasebokogyo-kosen"
        },
        {
          "display": "大村",
          "score": 0,
          "won": false,
          "name": "大村高校",
          "slug": "omura"
        }
      ]
    },
    {
      "date": "2026-03-20",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "佐世保南",
          "score": 6,
          "won": true,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        },
        {
          "display": "諫早東・口加・島原翔南・島原農",
          "score": 4,
          "won": false,
          "name": "諫早東・口加・島原翔南・島原農",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2026-03-20",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "猶興館",
          "score": 5,
          "won": true,
          "name": "猶興館高校",
          "slug": "yukokan"
        },
        {
          "display": "島原工",
          "score": 4,
          "won": false,
          "name": "島原工業高校",
          "slug": "shimabarakogyo"
        }
      ]
    },
    {
      "date": "2026-03-21",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "壱岐商",
          "score": 7,
          "won": false,
          "name": "壱岐商業高校",
          "slug": "ikishogyo"
        },
        {
          "display": "鎮西学院",
          "score": 8,
          "won": true,
          "name": "鎮西学院",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-21",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "上五島",
          "score": 4,
          "won": false,
          "name": "上五島高校",
          "slug": "kamigoto"
        },
        {
          "display": "壱岐",
          "score": 13,
          "won": true,
          "name": "壱岐高校",
          "slug": "iki"
        }
      ]
    },
    {
      "date": "2026-03-21",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "対馬",
          "score": 9,
          "won": false,
          "name": "対馬高校",
          "slug": "nagasaki-tsushima"
        },
        {
          "display": "佐世保実",
          "score": 10,
          "won": true,
          "name": "佐世保実",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-21",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "西彼杵・上対馬・長崎鶴洋",
          "score": 0,
          "won": false,
          "name": "西彼杵・上対馬・長崎鶴洋",
          "slug": null,
          "combined": true
        },
        {
          "display": "長崎北陽台",
          "score": 9,
          "won": true,
          "name": "長崎北陽台高校",
          "slug": "nagasakihokuyodai"
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "創成館",
          "score": 3,
          "won": true,
          "name": "創成館",
          "slug": null
        },
        {
          "display": "長崎工",
          "score": 1,
          "won": false,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "佐世保高専",
          "score": 0,
          "won": false,
          "name": "佐世保工業高専",
          "slug": "sasebokogyo-kosen"
        },
        {
          "display": "九州文化",
          "score": 7,
          "won": true,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "大村工",
          "score": 9,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        },
        {
          "display": "壱岐",
          "score": 2,
          "won": false,
          "name": "壱岐高校",
          "slug": "iki"
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "猶興館",
          "score": 1,
          "won": false,
          "name": "猶興館高校",
          "slug": "yukokan"
        },
        {
          "display": "長崎商",
          "score": 17,
          "won": true,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "波佐見",
          "score": 11,
          "won": true,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "長総大附",
          "score": 2,
          "won": false,
          "name": "長総大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "鎮西学院",
          "score": 0,
          "won": false,
          "name": "鎮西学院",
          "slug": null
        },
        {
          "display": "大崎",
          "score": 4,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎北陽台",
          "score": 4,
          "won": false,
          "name": "長崎北陽台高校",
          "slug": "nagasakihokuyodai"
        },
        {
          "display": "諫早農",
          "score": 12,
          "won": true,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        }
      ]
    },
    {
      "date": "2026-03-26",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "鹿町工",
          "score": 0,
          "won": false,
          "name": "鹿町工業高校",
          "slug": "shikamachikogyo"
        },
        {
          "display": "瓊浦",
          "score": 4,
          "won": true,
          "name": "瓊浦",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "西陵",
          "score": 6,
          "won": true,
          "name": "西陵高校",
          "slug": "nagasaki-seiryo"
        },
        {
          "display": "長崎南",
          "score": 4,
          "won": false,
          "name": "長崎南高校",
          "slug": "nagasakiminami"
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "佐世保実",
          "score": 1,
          "won": false,
          "name": "佐世保実",
          "slug": null
        },
        {
          "display": "清峰",
          "score": 7,
          "won": true,
          "name": "清峰高校",
          "slug": "nagasaki-seiho"
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "佐世保西・佐世保商・平戸",
          "score": 1,
          "won": false,
          "name": "佐世保西・佐世保商・平戸",
          "slug": null,
          "combined": true
        },
        {
          "display": "佐世保南",
          "score": 38,
          "won": true,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎明誠",
          "score": 2,
          "won": false,
          "name": "長崎明誠高校",
          "slug": "nagasakimeisei"
        },
        {
          "display": "島原中央",
          "score": 9,
          "won": true,
          "name": "島原中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "島原",
          "score": 0,
          "won": false,
          "name": "島原高校",
          "slug": "shimabara"
        },
        {
          "display": "海星",
          "score": 10,
          "won": true,
          "name": "海星",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎南山",
          "score": 4,
          "won": true,
          "name": "長崎南山",
          "slug": null
        },
        {
          "display": "佐世保工",
          "score": 2,
          "won": false,
          "name": "佐世保工業高校",
          "slug": "sasebokogyo"
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "川棚",
          "score": 4,
          "won": true,
          "name": "川棚高校",
          "slug": "kawatana"
        },
        {
          "display": "西海学園",
          "score": 2,
          "won": false,
          "name": "西海学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-27",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎東",
          "score": 3,
          "won": false,
          "name": "長崎東高校",
          "slug": "nagasakihigashi"
        },
        {
          "display": "諫早",
          "score": 13,
          "won": true,
          "name": "諫早高校",
          "slug": "isahaya"
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "清峰",
          "score": 1,
          "won": false,
          "name": "清峰高校",
          "slug": "nagasaki-seiho"
        },
        {
          "display": "九州文化",
          "score": 2,
          "won": true,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎商",
          "score": 3,
          "won": true,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        },
        {
          "display": "長崎南山",
          "score": 1,
          "won": false,
          "name": "長崎南山",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "大崎",
          "score": 9,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        },
        {
          "display": "佐世保南",
          "score": 1,
          "won": false,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "川棚",
          "score": 3,
          "won": false,
          "name": "川棚高校",
          "slug": "kawatana"
        },
        {
          "display": "創成館",
          "score": 11,
          "won": true,
          "name": "創成館",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "西陵",
          "score": 0,
          "won": false,
          "name": "西陵高校",
          "slug": "nagasaki-seiryo"
        },
        {
          "display": "瓊浦",
          "score": 4,
          "won": true,
          "name": "瓊浦",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "波佐見",
          "score": 5,
          "won": false,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "海星",
          "score": 6,
          "won": true,
          "name": "海星",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "諫早農",
          "score": 11,
          "won": true,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        },
        {
          "display": "島原中央",
          "score": 4,
          "won": false,
          "name": "島原中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-03-29",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "大村工",
          "score": 10,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        },
        {
          "display": "諫早",
          "score": 0,
          "won": false,
          "name": "諫早高校",
          "slug": "isahaya"
        }
      ]
    },
    {
      "date": "2026-03-31",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準決勝",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "諫早農",
          "score": 0,
          "won": false,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        },
        {
          "display": "大崎",
          "score": 10,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2026-03-31",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "瓊浦",
          "score": 6,
          "won": true,
          "name": "瓊浦",
          "slug": null
        },
        {
          "display": "長崎商",
          "score": 2,
          "won": false,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        }
      ]
    },
    {
      "date": "2026-03-31",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "大村工",
          "score": 1,
          "won": false,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        },
        {
          "display": "創成館",
          "score": 3,
          "won": true,
          "name": "創成館",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-04",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "準決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "瓊浦",
          "score": 0,
          "won": false,
          "name": "瓊浦",
          "slug": null
        },
        {
          "display": "大崎",
          "score": 1,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2026-04-05",
      "season": "spring",
      "tournament": "第158回 九州地区高等学校野球長崎県大会",
      "round": "決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "大崎",
          "score": 3,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        },
        {
          "display": "九州文化",
          "score": 0,
          "won": false,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-06",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "西彼杵・上対馬・長崎鶴洋",
          "score": 0,
          "won": false,
          "name": "西彼杵・上対馬・長崎鶴洋",
          "slug": null,
          "combined": true
        },
        {
          "display": "壱岐商",
          "score": 10,
          "won": true,
          "name": "壱岐商業高校",
          "slug": "ikishogyo"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "上五島",
          "score": 0,
          "won": false,
          "name": "上五島高校",
          "slug": "kamigoto"
        },
        {
          "display": "佐世保南",
          "score": 6,
          "won": true,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "島原中央",
          "score": 9,
          "won": true,
          "name": "島原中央",
          "slug": null
        },
        {
          "display": "対馬",
          "score": 7,
          "won": false,
          "name": "対馬高校",
          "slug": "nagasaki-tsushima"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "五島",
          "score": 1,
          "won": false,
          "name": "五島高校",
          "slug": "goto"
        },
        {
          "display": "諫早農",
          "score": 12,
          "won": true,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "壱岐",
          "score": 3,
          "won": false,
          "name": "壱岐高校",
          "slug": "iki"
        },
        {
          "display": "鎮西学院",
          "score": 7,
          "won": true,
          "name": "鎮西学院",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "島原翔南・島原農",
          "score": 0,
          "won": false,
          "name": "島原翔南・島原農",
          "slug": null,
          "combined": true
        },
        {
          "display": "鹿町工",
          "score": 27,
          "won": true,
          "name": "鹿町工業高校",
          "slug": "shikamachikogyo"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "口加",
          "score": 1,
          "won": false,
          "name": "口加高校",
          "slug": "koka"
        },
        {
          "display": "長崎北陽台",
          "score": 2,
          "won": true,
          "name": "長崎北陽台高校",
          "slug": "nagasakihokuyodai"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "猶興館",
          "score": 2,
          "won": false,
          "name": "猶興館高校",
          "slug": "yukokan"
        },
        {
          "display": "大村工",
          "score": 3,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        }
      ]
    },
    {
      "date": "2026-07-07",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "西陵",
          "score": 0,
          "won": false,
          "name": "西陵高校",
          "slug": "nagasaki-seiryo"
        },
        {
          "display": "長崎商",
          "score": 9,
          "won": true,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        }
      ]
    },
    {
      "date": "2026-07-08",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "佐世保工",
          "score": 2,
          "won": false,
          "name": "佐世保工業高校",
          "slug": "sasebokogyo"
        },
        {
          "display": "大村",
          "score": 3,
          "won": true,
          "name": "大村高校",
          "slug": "omura"
        }
      ]
    },
    {
      "date": "2026-07-08",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎東",
          "score": 4,
          "won": true,
          "name": "長崎東高校",
          "slug": "nagasakihigashi"
        },
        {
          "display": "小浜",
          "score": 1,
          "won": false,
          "name": "小浜高校",
          "slug": "obama"
        }
      ]
    },
    {
      "date": "2026-07-08",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎北",
          "score": 7,
          "won": true,
          "name": "長崎北高校",
          "slug": "nagasakikita"
        },
        {
          "display": "佐世保高専",
          "score": 4,
          "won": false,
          "name": "佐世保工業高専",
          "slug": "sasebokogyo-kosen"
        }
      ]
    },
    {
      "date": "2026-07-08",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "島原",
          "score": 0,
          "won": false,
          "name": "島原高校",
          "slug": "shimabara"
        },
        {
          "display": "西海学園",
          "score": 7,
          "won": true,
          "name": "西海学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-08",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎明誠",
          "score": 2,
          "won": false,
          "name": "長崎明誠高校",
          "slug": "nagasakimeisei"
        },
        {
          "display": "諫早",
          "score": 4,
          "won": true,
          "name": "諫早高校",
          "slug": "isahaya"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "大崎",
          "score": 10,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        },
        {
          "display": "長崎北陽台",
          "score": 0,
          "won": false,
          "name": "長崎北陽台高校",
          "slug": "nagasakihokuyodai"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "壱岐商",
          "score": 0,
          "won": false,
          "name": "壱岐商業高校",
          "slug": "ikishogyo"
        },
        {
          "display": "長崎日大",
          "score": 10,
          "won": true,
          "name": "長崎日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 6,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "鹿町工",
          "score": 1,
          "won": false,
          "name": "鹿町工業高校",
          "slug": "shikamachikogyo"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎商",
          "score": 1,
          "won": false,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        },
        {
          "display": "九州文化",
          "score": 7,
          "won": true,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "大村工",
          "score": 5,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        },
        {
          "display": "瓊浦",
          "score": 3,
          "won": false,
          "name": "瓊浦",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "波佐見",
          "score": 12,
          "won": true,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "諫早",
          "score": 0,
          "won": false,
          "name": "諫早高校",
          "slug": "isahaya"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎工",
          "score": 3,
          "won": true,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        },
        {
          "display": "島原工",
          "score": 1,
          "won": false,
          "name": "島原工業高校",
          "slug": "shimabarakogyo"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎南",
          "score": 3,
          "won": false,
          "name": "長崎南高校",
          "slug": "nagasakiminami"
        },
        {
          "display": "大村",
          "score": 4,
          "won": true,
          "name": "大村高校",
          "slug": "omura"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "川棚",
          "score": 7,
          "won": true,
          "name": "川棚高校",
          "slug": "kawatana"
        },
        {
          "display": "長崎北",
          "score": 6,
          "won": false,
          "name": "長崎北高校",
          "slug": "nagasakikita"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "佐世保南",
          "score": 11,
          "won": true,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        },
        {
          "display": "諫早東",
          "score": 0,
          "won": false,
          "name": "諫早東高校",
          "slug": "isahayahigashi"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎東",
          "score": 9,
          "won": false,
          "name": "長崎東高校",
          "slug": "nagasakihigashi"
        },
        {
          "display": "佐世保北",
          "score": 10,
          "won": true,
          "name": "佐世保北高校",
          "slug": "sasebokita"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "清峰",
          "score": 1,
          "won": false,
          "name": "清峰高校",
          "slug": "nagasaki-seiho"
        },
        {
          "display": "諫早農",
          "score": 2,
          "won": true,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎南山",
          "score": 1,
          "won": false,
          "name": "長崎南山",
          "slug": null
        },
        {
          "display": "大崎",
          "score": 8,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "波佐見",
          "score": 4,
          "won": true,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "佐世保南",
          "score": 0,
          "won": false,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 6,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "諫早農",
          "score": 3,
          "won": false,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "九州文化",
          "score": 8,
          "won": true,
          "name": "九州文化",
          "slug": null
        },
        {
          "display": "大村",
          "score": 1,
          "won": false,
          "name": "大村高校",
          "slug": "omura"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎工",
          "score": 4,
          "won": true,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        },
        {
          "display": "海星",
          "score": 2,
          "won": false,
          "name": "海星",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "川棚",
          "score": 0,
          "won": false,
          "name": "川棚高校",
          "slug": "kawatana"
        },
        {
          "display": "大村工",
          "score": 5,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "佐世保北",
          "score": 0,
          "won": false,
          "name": "佐世保北高校",
          "slug": "sasebokita"
        },
        {
          "display": "創成館",
          "score": 2,
          "won": true,
          "name": "創成館",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-20",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "準々決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "創成館",
          "score": 1,
          "won": false,
          "name": "創成館",
          "slug": null
        },
        {
          "display": "大崎",
          "score": 4,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2026-07-20",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "準々決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎工",
          "score": 3,
          "won": false,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        },
        {
          "display": "長崎日大",
          "score": 4,
          "won": true,
          "name": "長崎日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-20",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "準々決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "波佐見",
          "score": 11,
          "won": true,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "九州文化",
          "score": 6,
          "won": false,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-20",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "準々決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 10,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "大村工",
          "score": 1,
          "won": false,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        }
      ]
    },
    {
      "date": "2026-07-23",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "準決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "波佐見",
          "score": 4,
          "won": false,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "長崎日大",
          "score": 14,
          "won": true,
          "name": "長崎日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-23",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "準決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 2,
          "won": false,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "大崎",
          "score": 5,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2026-07-25",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権長崎大会",
      "round": "決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎日大",
          "score": 7,
          "won": true,
          "name": "長崎日大",
          "slug": null
        },
        {
          "display": "大崎",
          "score": 2,
          "won": false,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎北陽台",
          "score": 3,
          "won": false,
          "name": "長崎北陽台高校",
          "slug": "nagasakihokuyodai"
        },
        {
          "display": "大村",
          "score": 6,
          "won": true,
          "name": "大村高校",
          "slug": "omura"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "諫早東・口加・島原翔南・島原農",
          "score": 2,
          "won": false,
          "name": "諫早東・口加・島原翔南・島原農",
          "slug": null,
          "combined": true
        },
        {
          "display": "佐世保工",
          "score": 3,
          "won": true,
          "name": "佐世保工業高校",
          "slug": "sasebokogyo"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "清峰",
          "score": 3,
          "won": true,
          "name": "清峰高校",
          "slug": "nagasaki-seiho"
        },
        {
          "display": "佐世保北",
          "score": 2,
          "won": false,
          "name": "佐世保北高校",
          "slug": "sasebokita"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "佐世保西・佐世保商・平戸",
          "score": 0,
          "won": false,
          "name": "佐世保西・佐世保商・平戸",
          "slug": null,
          "combined": true
        },
        {
          "display": "小浜",
          "score": 22,
          "won": true,
          "name": "小浜高校",
          "slug": "obama"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎南山",
          "score": 7,
          "won": true,
          "name": "長崎南山",
          "slug": null
        },
        {
          "display": "対馬",
          "score": 0,
          "won": false,
          "name": "対馬高校",
          "slug": "nagasaki-tsushima"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "西彼杵・上対馬・長崎鶴洋",
          "score": 0,
          "won": false,
          "name": "西彼杵・上対馬・長崎鶴洋",
          "slug": null,
          "combined": true
        },
        {
          "display": "大崎",
          "score": 14,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "猶興館",
          "score": 1,
          "won": false,
          "name": "猶興館高校",
          "slug": "yukokan"
        },
        {
          "display": "壱岐商",
          "score": 5,
          "won": true,
          "name": "壱岐商業高校",
          "slug": "ikishogyo"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "川棚",
          "score": 3,
          "won": false,
          "name": "川棚高校",
          "slug": "kawatana"
        },
        {
          "display": "佐世保高専",
          "score": 5,
          "won": true,
          "name": "佐世保工業高専",
          "slug": "sasebokogyo-kosen"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎東",
          "score": 4,
          "won": false,
          "name": "長崎東高校",
          "slug": "nagasakihigashi"
        },
        {
          "display": "九州文化",
          "score": 5,
          "won": true,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "1回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "島原",
          "score": 10,
          "won": true,
          "name": "島原高校",
          "slug": "shimabara"
        },
        {
          "display": "長崎北",
          "score": 5,
          "won": false,
          "name": "長崎北高校",
          "slug": "nagasakikita"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "壱岐商",
          "score": 1,
          "won": false,
          "name": "壱岐商業高校",
          "slug": "ikishogyo"
        },
        {
          "display": "波佐見",
          "score": 2,
          "won": true,
          "name": "波佐見高校",
          "slug": "hasami"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "小浜",
          "score": 1,
          "won": false,
          "name": "小浜高校",
          "slug": "obama"
        },
        {
          "display": "創成館",
          "score": 2,
          "won": true,
          "name": "創成館",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "島原",
          "score": 0,
          "won": false,
          "name": "島原高校",
          "slug": "shimabara"
        },
        {
          "display": "長崎商",
          "score": 4,
          "won": true,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "清峰",
          "score": 0,
          "won": false,
          "name": "清峰高校",
          "slug": "nagasaki-seiho"
        },
        {
          "display": "大村工",
          "score": 2,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "瓊浦",
          "score": 8,
          "won": true,
          "name": "瓊浦",
          "slug": null
        },
        {
          "display": "鹿町工",
          "score": 1,
          "won": false,
          "name": "鹿町工業高校",
          "slug": "shikamachikogyo"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "上五島",
          "score": 2,
          "won": false,
          "name": "上五島高校",
          "slug": "kamigoto"
        },
        {
          "display": "九州文化",
          "score": 16,
          "won": true,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "大村",
          "score": 2,
          "won": false,
          "name": "大村高校",
          "slug": "omura"
        },
        {
          "display": "長崎日大",
          "score": 5,
          "won": true,
          "name": "長崎日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長総大附",
          "score": 1,
          "won": false,
          "name": "長総大附",
          "slug": null
        },
        {
          "display": "大崎",
          "score": 5,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "壱岐",
          "score": 3,
          "won": true,
          "name": "壱岐高校",
          "slug": "iki"
        },
        {
          "display": "島原工",
          "score": 0,
          "won": false,
          "name": "島原工業高校",
          "slug": "shimabarakogyo"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "たちばなしんきんフィールド・オブ・ドリームス",
      "teams": [
        {
          "display": "長崎明誠",
          "score": 1,
          "won": false,
          "name": "長崎明誠高校",
          "slug": "nagasakimeisei"
        },
        {
          "display": "西陵",
          "score": 7,
          "won": true,
          "name": "西陵高校",
          "slug": "nagasaki-seiryo"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎南",
          "score": 3,
          "won": false,
          "name": "長崎南高校",
          "slug": "nagasakiminami"
        },
        {
          "display": "鎮西学院",
          "score": 10,
          "won": true,
          "name": "鎮西学院",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "諫早農",
          "score": 8,
          "won": true,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        },
        {
          "display": "佐世保南",
          "score": 1,
          "won": false,
          "name": "佐世保南高校",
          "slug": "sasebominami"
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "佐世保高専",
          "score": 2,
          "won": false,
          "name": "佐世保工業高専",
          "slug": "sasebokogyo-kosen"
        },
        {
          "display": "諫早",
          "score": 9,
          "won": true,
          "name": "諫早高校",
          "slug": "isahaya"
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎工",
          "score": 8,
          "won": true,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        },
        {
          "display": "佐世保工",
          "score": 0,
          "won": false,
          "name": "佐世保工業高校",
          "slug": "sasebokogyo"
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "2回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "長崎南山",
          "score": 1,
          "won": false,
          "name": "長崎南山",
          "slug": null
        },
        {
          "display": "長崎西",
          "score": 3,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "諫早",
          "score": 0,
          "won": false,
          "name": "諫早高校",
          "slug": "isahaya"
        },
        {
          "display": "創成館",
          "score": 10,
          "won": true,
          "name": "創成館",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎工",
          "score": 4,
          "won": false,
          "name": "長崎工業高校",
          "slug": "nagasakikogyo"
        },
        {
          "display": "大村工",
          "score": 11,
          "won": true,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "西陵",
          "score": 1,
          "won": false,
          "name": "西陵高校",
          "slug": "nagasaki-seiryo"
        },
        {
          "display": "大崎",
          "score": 8,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2025-09-28",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 3,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "海星",
          "score": 2,
          "won": false,
          "name": "海星",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-28",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "瓊浦",
          "score": 5,
          "won": true,
          "name": "瓊浦",
          "slug": null
        },
        {
          "display": "壱岐",
          "score": 4,
          "won": false,
          "name": "壱岐高校",
          "slug": "iki"
        }
      ]
    },
    {
      "date": "2025-09-28",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "波佐見",
          "score": 2,
          "won": false,
          "name": "波佐見高校",
          "slug": "hasami"
        },
        {
          "display": "九州文化",
          "score": 3,
          "won": true,
          "name": "九州文化",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-28",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "3回戦",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "諫早農",
          "score": 3,
          "won": false,
          "name": "諫早農業高校",
          "slug": "isahayanogyo"
        },
        {
          "display": "長崎商",
          "score": 8,
          "won": true,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        }
      ]
    },
    {
      "date": "2025-10-04",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "瓊浦",
          "score": 1,
          "won": false,
          "name": "瓊浦",
          "slug": null
        },
        {
          "display": "長崎西",
          "score": 5,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        }
      ]
    },
    {
      "date": "2025-10-04",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "大村工",
          "score": 3,
          "won": false,
          "name": "大村工業高校",
          "slug": "omurakogyo"
        },
        {
          "display": "大崎",
          "score": 4,
          "won": true,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2025-10-04",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "準々決勝",
      "venue": "長崎県営野球場",
      "teams": [
        {
          "display": "九州文化",
          "score": 4,
          "won": true,
          "name": "九州文化",
          "slug": null
        },
        {
          "display": "長崎商",
          "score": 2,
          "won": false,
          "name": "長崎商業高校",
          "slug": "nagasakishogyo"
        }
      ]
    },
    {
      "date": "2025-10-11",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "準決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 4,
          "won": true,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "大崎",
          "score": 3,
          "won": false,
          "name": "大崎高校",
          "slug": "nagasaki-osaki"
        }
      ]
    },
    {
      "date": "2025-10-13",
      "season": "autumn",
      "tournament": "第157回九州地区高等学校野球長崎県大会",
      "round": "決勝",
      "venue": "佐世保市総合グラウンド野球場",
      "teams": [
        {
          "display": "長崎西",
          "score": 1,
          "won": false,
          "name": "長崎西高校",
          "slug": "nagasakinishi"
        },
        {
          "display": "長崎日大",
          "score": 3,
          "won": true,
          "name": "長崎日大",
          "slug": null
        }
      ]
    }
  ]
};
