// このファイルは scripts/build-regional-results.mjs が生成する。直接編集しない。
// 出典: 新潟県高等学校野球連盟（https://niigata-hbf.jp/）

import type { RegionalDistrict } from "@/lib/regional-results";

export const REGIONAL_NIIGATA: RegionalDistrict = {
  "slug": "niigata",
  "district": "新潟",
  "sourceName": "新潟県高等学校野球連盟",
  "sourceUrl": "https://niigata-hbf.jp/",
  "games": [
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "加茂",
          "score": 0,
          "won": false,
          "name": "加茂高校",
          "slug": "kamo"
        },
        {
          "display": "開志学園",
          "score": 12,
          "won": true,
          "name": "開志学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "新潟工",
          "score": 5,
          "won": false,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        },
        {
          "display": "佐渡",
          "score": 9,
          "won": true,
          "name": "佐渡高校",
          "slug": "sado"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "新津工",
          "score": 4,
          "won": false,
          "name": "新津工業高校",
          "slug": "niitsukogyo"
        },
        {
          "display": "新発田農",
          "score": 14,
          "won": true,
          "name": "新発田農業高校",
          "slug": "shibatanogyo"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "万代",
          "score": 1,
          "won": false,
          "name": "万代高校",
          "slug": "bandai"
        },
        {
          "display": "東京学館新潟",
          "score": 12,
          "won": true,
          "name": "東京学館新潟",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "加茂農林",
          "score": 0,
          "won": false,
          "name": "加茂農林高校",
          "slug": "kamonorin"
        },
        {
          "display": "新発田南",
          "score": 42,
          "won": true,
          "name": "新発田南高校",
          "slug": "shibataminami"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "6校連合",
          "score": 1,
          "won": false,
          "name": "6校連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "三条",
          "score": 9,
          "won": true,
          "name": "三条高校",
          "slug": "sanjo"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "小千谷西",
          "score": 1,
          "won": false,
          "name": "小千谷西高校",
          "slug": "ojiyanishi"
        },
        {
          "display": "長岡",
          "score": 6,
          "won": true,
          "name": "長岡高校",
          "slug": "nagaoka"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "三条東",
          "score": 2,
          "won": false,
          "name": "三条東高校",
          "slug": "sanjohigashi"
        },
        {
          "display": "新潟産大附",
          "score": 18,
          "won": true,
          "name": "新潟産大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "高田",
          "score": 0,
          "won": false,
          "name": "高田高校",
          "slug": "niigata-takata"
        },
        {
          "display": "小千谷",
          "score": 1,
          "won": true,
          "name": "小千谷高校",
          "slug": "ojiya"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "常総新商嶺",
          "score": 0,
          "won": false,
          "name": "常総新商嶺",
          "slug": null
        },
        {
          "display": "十日町",
          "score": 10,
          "won": true,
          "name": "十日町高校",
          "slug": "tokamachi"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡商",
          "score": 22,
          "won": true,
          "name": "長岡商業高校",
          "slug": "nagaokashogyo"
        },
        {
          "display": "高田農",
          "score": 1,
          "won": false,
          "name": "高田農業高校",
          "slug": "takadanogyo"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "新潟第一",
          "score": 3,
          "won": false,
          "name": "新潟第一",
          "slug": null
        },
        {
          "display": "新潟西",
          "score": 5,
          "won": true,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "五泉",
          "score": 0,
          "won": false,
          "name": "五泉高校",
          "slug": "gosen"
        },
        {
          "display": "北越",
          "score": 10,
          "won": true,
          "name": "北越",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "新東向商",
          "score": 6,
          "won": false,
          "name": "新東向商",
          "slug": null
        },
        {
          "display": "巻総合",
          "score": 13,
          "won": true,
          "name": "巻総合高校",
          "slug": "makisogo"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟明訓",
          "score": 13,
          "won": true,
          "name": "新潟明訓",
          "slug": null
        },
        {
          "display": "新潟",
          "score": 4,
          "won": false,
          "name": "新潟高校",
          "slug": "niigata"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "敬和学園",
          "score": 1,
          "won": false,
          "name": "敬和学園",
          "slug": null
        },
        {
          "display": "村上桜ケ丘",
          "score": 11,
          "won": true,
          "name": "村上桜ヶ丘高校",
          "slug": "murakamisakuragaoka"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟江南",
          "score": 2,
          "won": false,
          "name": "新潟江南高校",
          "slug": "niigatakonan"
        },
        {
          "display": "加茂暁星",
          "score": 12,
          "won": true,
          "name": "加茂暁星",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡大手",
          "score": 13,
          "won": true,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "長岡向陵",
          "score": 6,
          "won": false,
          "name": "長岡向陵高校",
          "slug": "nagaokakoryo"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "十総・塩沢",
          "score": 10,
          "won": true,
          "name": "十総・塩沢",
          "slug": null,
          "combined": true
        },
        {
          "display": "長岡工",
          "score": 6,
          "won": false,
          "name": "長岡工業高校",
          "slug": "nagaokakogyo"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "上越総合技術",
          "score": 2,
          "won": false,
          "name": "上越総合技術高校",
          "slug": "joetsusogogijutsu"
        },
        {
          "display": "六日町",
          "score": 5,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "糸魚川",
          "score": 10,
          "won": true,
          "name": "糸魚川高校",
          "slug": "itoigawa"
        },
        {
          "display": "上越",
          "score": 2,
          "won": false,
          "name": "上越",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-27",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "佐渡総合",
          "score": 0,
          "won": false,
          "name": "佐渡総合高校",
          "slug": "sadosogo"
        },
        {
          "display": "新発田中央",
          "score": 16,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-27",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新津",
          "score": 0,
          "won": false,
          "name": "新津高校",
          "slug": "niitsu"
        },
        {
          "display": "新発田",
          "score": 10,
          "won": true,
          "name": "新発田高校",
          "slug": "niigata-shibata"
        }
      ]
    },
    {
      "date": "2026-04-28",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "みどりと森",
      "teams": [
        {
          "display": "巻",
          "score": 3,
          "won": false,
          "name": "巻高校",
          "slug": "maki"
        },
        {
          "display": "新潟南",
          "score": 5,
          "won": true,
          "name": "新潟南高校",
          "slug": "niigataminami"
        }
      ]
    },
    {
      "date": "2026-04-28",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "北村上中条碧",
          "score": 2,
          "won": false,
          "name": "北村上中条碧",
          "slug": null
        },
        {
          "display": "新潟商",
          "score": 6,
          "won": true,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        }
      ]
    },
    {
      "date": "2026-04-28",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "新潟県央工",
          "score": 9,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        },
        {
          "display": "柏崎",
          "score": 2,
          "won": false,
          "name": "柏崎高校",
          "slug": "kashiwazaki"
        }
      ]
    },
    {
      "date": "2026-04-28",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "小出",
          "score": 1,
          "won": false,
          "name": "小出高校",
          "slug": "koide"
        },
        {
          "display": "柏崎工",
          "score": 9,
          "won": true,
          "name": "柏崎工業高校",
          "slug": "kashiwazakikogyo"
        }
      ]
    },
    {
      "date": "2026-04-28",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡高専",
          "score": 0,
          "won": false,
          "name": "長岡工業高専",
          "slug": "nagaokakogyo-kosen"
        },
        {
          "display": "高田北城",
          "score": 11,
          "won": true,
          "name": "高田北城高校",
          "slug": "takadahokujo"
        }
      ]
    },
    {
      "date": "2026-04-28",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "海洋",
          "score": 0,
          "won": false,
          "name": "海洋高校",
          "slug": "niigata-kaiyo"
        },
        {
          "display": "関根学園",
          "score": 10,
          "won": true,
          "name": "関根学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新発田南",
          "score": 2,
          "won": false,
          "name": "新発田南高校",
          "slug": "shibataminami"
        },
        {
          "display": "日本文理",
          "score": 11,
          "won": true,
          "name": "日本文理",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟明訓",
          "score": 12,
          "won": true,
          "name": "新潟明訓",
          "slug": null
        },
        {
          "display": "村上桜ケ丘",
          "score": 1,
          "won": false,
          "name": "村上桜ヶ丘高校",
          "slug": "murakamisakuragaoka"
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "開志学園",
          "score": 3,
          "won": true,
          "name": "開志学園",
          "slug": null
        },
        {
          "display": "新発田農",
          "score": 1,
          "won": false,
          "name": "新発田農業高校",
          "slug": "shibatanogyo"
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "新潟西",
          "score": 0,
          "won": false,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        },
        {
          "display": "北越",
          "score": 13,
          "won": true,
          "name": "北越",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "中越",
          "score": 11,
          "won": true,
          "name": "中越",
          "slug": null
        },
        {
          "display": "長岡商",
          "score": 2,
          "won": false,
          "name": "長岡商業高校",
          "slug": "nagaokashogyo"
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "帝京長岡",
          "score": 9,
          "won": true,
          "name": "帝京長岡",
          "slug": null
        },
        {
          "display": "三条",
          "score": 2,
          "won": false,
          "name": "三条高校",
          "slug": "sanjo"
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡",
          "score": 8,
          "won": true,
          "name": "長岡高校",
          "slug": "nagaoka"
        },
        {
          "display": "小千谷",
          "score": 0,
          "won": false,
          "name": "小千谷高校",
          "slug": "ojiya"
        }
      ]
    },
    {
      "date": "2026-05-01",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "十日町",
          "score": 2,
          "won": false,
          "name": "十日町高校",
          "slug": "tokamachi"
        },
        {
          "display": "新潟産大附",
          "score": 3,
          "won": true,
          "name": "新潟産大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新発田中央",
          "score": 10,
          "won": true,
          "name": "新発田中央",
          "slug": null
        },
        {
          "display": "新潟南",
          "score": 0,
          "won": false,
          "name": "新潟南高校",
          "slug": "niigataminami"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟商",
          "score": 14,
          "won": true,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        },
        {
          "display": "新発田",
          "score": 3,
          "won": false,
          "name": "新発田高校",
          "slug": "niigata-shibata"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "加茂暁星",
          "score": 12,
          "won": true,
          "name": "加茂暁星",
          "slug": null
        },
        {
          "display": "佐渡",
          "score": 0,
          "won": false,
          "name": "佐渡高校",
          "slug": "sado"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "巻総合",
          "score": 1,
          "won": false,
          "name": "巻総合高校",
          "slug": "makisogo"
        },
        {
          "display": "東京学館新潟",
          "score": 8,
          "won": true,
          "name": "東京学館新潟",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "柏崎工",
          "score": 1,
          "won": false,
          "name": "柏崎工業高校",
          "slug": "kashiwazakikogyo"
        },
        {
          "display": "新潟県央工",
          "score": 12,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "十総・塩沢",
          "score": 0,
          "won": false,
          "name": "十総・塩沢",
          "slug": null,
          "combined": true
        },
        {
          "display": "六日町",
          "score": 10,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡大手",
          "score": 9,
          "won": true,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "糸魚川",
          "score": 4,
          "won": false,
          "name": "糸魚川高校",
          "slug": "itoigawa"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "高田北城",
          "score": 2,
          "won": false,
          "name": "高田北城高校",
          "slug": "takadahokujo"
        },
        {
          "display": "関根学園",
          "score": 3,
          "won": true,
          "name": "関根学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-04",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "3回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "新潟商",
          "score": 3,
          "won": true,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        },
        {
          "display": "六日町",
          "score": 2,
          "won": false,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2026-05-05",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "長岡",
          "score": 2,
          "won": false,
          "name": "長岡高校",
          "slug": "nagaoka"
        },
        {
          "display": "日本文理",
          "score": 3,
          "won": true,
          "name": "日本文理",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-05",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "3回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "東京学館新潟",
          "score": 10,
          "won": false,
          "name": "東京学館新潟",
          "slug": null
        },
        {
          "display": "新潟県央工",
          "score": 11,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        }
      ]
    },
    {
      "date": "2026-05-05",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "3回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡大手",
          "score": 3,
          "won": false,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "新発田中央",
          "score": 10,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-06",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "準々決勝",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟商",
          "score": 6,
          "won": false,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        },
        {
          "display": "新潟明訓",
          "score": 13,
          "won": true,
          "name": "新潟明訓",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-06",
      "season": "spring",
      "tournament": "第154回北信越地区高等学校野球新潟県大会（令和8年度春季）",
      "round": "準々決勝",
      "venue": "悠久山",
      "teams": [
        {
          "display": "新潟産大附",
          "score": 11,
          "won": true,
          "name": "新潟産大附",
          "slug": null
        },
        {
          "display": "新潟県央工",
          "score": 3,
          "won": false,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        }
      ]
    },
    {
      "date": "2026-07-09",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "1回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "佐渡総合",
          "score": 17,
          "won": true,
          "name": "佐渡総合高校",
          "slug": "sadosogo"
        },
        {
          "display": "三条東",
          "score": 13,
          "won": false,
          "name": "三条東高校",
          "slug": "sanjohigashi"
        }
      ]
    },
    {
      "date": "2026-07-09",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "1回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "小千谷",
          "score": 1,
          "won": false,
          "name": "小千谷高校",
          "slug": "ojiya"
        },
        {
          "display": "巻総合",
          "score": 6,
          "won": true,
          "name": "巻総合高校",
          "slug": "makisogo"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "新潟明訓",
          "score": 12,
          "won": true,
          "name": "新潟明訓",
          "slug": null
        },
        {
          "display": "高田",
          "score": 2,
          "won": false,
          "name": "高田高校",
          "slug": "niigata-takata"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "新潟商",
          "score": 7,
          "won": true,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        },
        {
          "display": "新潟",
          "score": 3,
          "won": false,
          "name": "新潟高校",
          "slug": "niigata"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟青陵",
          "score": 1,
          "won": false,
          "name": "新潟青陵",
          "slug": null
        },
        {
          "display": "新発田農",
          "score": 7,
          "won": true,
          "name": "新発田農業高校",
          "slug": "shibatanogyo"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡向陵",
          "score": 1,
          "won": false,
          "name": "長岡向陵高校",
          "slug": "nagaokakoryo"
        },
        {
          "display": "上越",
          "score": 3,
          "won": true,
          "name": "上越",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "新潟産大附",
          "score": 10,
          "won": true,
          "name": "新潟産大附",
          "slug": null
        },
        {
          "display": "柏崎",
          "score": 1,
          "won": false,
          "name": "柏崎高校",
          "slug": "kashiwazaki"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "新潟東",
          "score": 4,
          "won": false,
          "name": "新潟東高校",
          "slug": "niigatahigashi"
        },
        {
          "display": "新津工",
          "score": 14,
          "won": true,
          "name": "新津工業高校",
          "slug": "niitsukogyo"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "六日町",
          "score": 7,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        },
        {
          "display": "三条",
          "score": 0,
          "won": false,
          "name": "三条高校",
          "slug": "sanjo"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "見附･三条商業･分水",
          "score": 1,
          "won": false,
          "name": "見附･三条商業･分水",
          "slug": null,
          "combined": true
        },
        {
          "display": "巻",
          "score": 9,
          "won": true,
          "name": "巻高校",
          "slug": "maki"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "新潟西",
          "score": 10,
          "won": true,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        },
        {
          "display": "長農正栃",
          "score": 0,
          "won": false,
          "name": "長農正栃",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "五泉",
          "score": 12,
          "won": true,
          "name": "五泉高校",
          "slug": "gosen"
        },
        {
          "display": "万代",
          "score": 2,
          "won": false,
          "name": "万代高校",
          "slug": "bandai"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "向陽新発商",
          "score": 2,
          "won": false,
          "name": "向陽新発商",
          "slug": null
        },
        {
          "display": "新潟南",
          "score": 9,
          "won": true,
          "name": "新潟南高校",
          "slug": "niigataminami"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "小千谷西",
          "score": 13,
          "won": true,
          "name": "小千谷西高校",
          "slug": "ojiyanishi"
        },
        {
          "display": "塩沢商工",
          "score": 0,
          "won": false,
          "name": "塩沢商工高校",
          "slug": "shiozawashoko"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "高田北城",
          "score": 2,
          "won": false,
          "name": "高田北城高校",
          "slug": "takadahokujo"
        },
        {
          "display": "加茂暁星",
          "score": 3,
          "won": true,
          "name": "加茂暁星",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "新津",
          "score": 2,
          "won": false,
          "name": "新津高校",
          "slug": "niitsu"
        },
        {
          "display": "長岡大手",
          "score": 9,
          "won": true,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟県央工",
          "score": 15,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        },
        {
          "display": "新潟江南",
          "score": 6,
          "won": false,
          "name": "新潟江南高校",
          "slug": "niigatakonan"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新発田",
          "score": 2,
          "won": false,
          "name": "新発田高校",
          "slug": "niigata-shibata"
        },
        {
          "display": "北越",
          "score": 9,
          "won": true,
          "name": "北越",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "帝京長岡",
          "score": 7,
          "won": true,
          "name": "帝京長岡",
          "slug": null
        },
        {
          "display": "長岡",
          "score": 3,
          "won": false,
          "name": "長岡高校",
          "slug": "nagaoka"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡高専",
          "score": 1,
          "won": false,
          "name": "長岡工業高専",
          "slug": "nagaokakogyo-kosen"
        },
        {
          "display": "新潟第一",
          "score": 10,
          "won": true,
          "name": "新潟第一",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "小出",
          "score": 1,
          "won": false,
          "name": "小出高校",
          "slug": "koide"
        },
        {
          "display": "新発田中央",
          "score": 8,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "海洋",
          "score": 1,
          "won": false,
          "name": "海洋高校",
          "slug": "niigata-kaiyo"
        },
        {
          "display": "十日町総合",
          "score": 7,
          "won": true,
          "name": "十日町総合高校",
          "slug": "tokamachisogo"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "上越総合技術",
          "score": 0,
          "won": false,
          "name": "上越総合技術高校",
          "slug": "joetsusogogijutsu"
        },
        {
          "display": "十日町",
          "score": 1,
          "won": true,
          "name": "十日町高校",
          "slug": "tokamachi"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "糸魚川",
          "score": 4,
          "won": false,
          "name": "糸魚川高校",
          "slug": "itoigawa"
        },
        {
          "display": "関根学園",
          "score": 5,
          "won": true,
          "name": "関根学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "長岡工",
          "score": 5,
          "won": false,
          "name": "長岡工業高校",
          "slug": "nagaokakogyo"
        },
        {
          "display": "巻総合",
          "score": 7,
          "won": true,
          "name": "巻総合高校",
          "slug": "makisogo"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "佐渡",
          "score": 4,
          "won": true,
          "name": "佐渡高校",
          "slug": "sado"
        },
        {
          "display": "長岡商",
          "score": 2,
          "won": false,
          "name": "長岡商業高校",
          "slug": "nagaokashogyo"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "加茂農林",
          "score": 0,
          "won": false,
          "name": "加茂農林高校",
          "slug": "kamonorin"
        },
        {
          "display": "佐渡総合",
          "score": 17,
          "won": true,
          "name": "佐渡総合高校",
          "slug": "sadosogo"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "高田農",
          "score": 5,
          "won": false,
          "name": "高田農業高校",
          "slug": "takadanogyo"
        },
        {
          "display": "新発田南",
          "score": 11,
          "won": true,
          "name": "新発田南高校",
          "slug": "shibataminami"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "柏崎工",
          "score": 4,
          "won": false,
          "name": "柏崎工業高校",
          "slug": "kashiwazakikogyo"
        },
        {
          "display": "新潟工",
          "score": 6,
          "won": true,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "加茂",
          "score": 7,
          "won": true,
          "name": "加茂高校",
          "slug": "kamo"
        },
        {
          "display": "村上桜ケ丘",
          "score": 0,
          "won": false,
          "name": "村上桜ヶ丘高校",
          "slug": "murakamisakuragaoka"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "新潟明訓",
          "score": 11,
          "won": true,
          "name": "新潟明訓",
          "slug": null
        },
        {
          "display": "新潟商",
          "score": 0,
          "won": false,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "新津工",
          "score": 0,
          "won": false,
          "name": "新津工業高校",
          "slug": "niitsukogyo"
        },
        {
          "display": "開志学園",
          "score": 9,
          "won": true,
          "name": "開志学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "日本文理",
          "score": 18,
          "won": true,
          "name": "日本文理",
          "slug": null
        },
        {
          "display": "新発田農",
          "score": 5,
          "won": false,
          "name": "新発田農業高校",
          "slug": "shibatanogyo"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "巻",
          "score": 2,
          "won": false,
          "name": "巻高校",
          "slug": "maki"
        },
        {
          "display": "六日町",
          "score": 3,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "新潟西",
          "score": 1,
          "won": false,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        },
        {
          "display": "五泉",
          "score": 11,
          "won": true,
          "name": "五泉高校",
          "slug": "gosen"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "新潟南",
          "score": 15,
          "won": true,
          "name": "新潟南高校",
          "slug": "niigataminami"
        },
        {
          "display": "小千谷西",
          "score": 8,
          "won": false,
          "name": "小千谷西高校",
          "slug": "ojiyanishi"
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "長岡大手",
          "score": 1,
          "won": false,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "加茂暁星",
          "score": 8,
          "won": true,
          "name": "加茂暁星",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "ハードオフ",
      "teams": [
        {
          "display": "十日町",
          "score": 2,
          "won": false,
          "name": "十日町高校",
          "slug": "tokamachi"
        },
        {
          "display": "関根学園",
          "score": 4,
          "won": true,
          "name": "関根学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "北越",
          "score": 7,
          "won": true,
          "name": "北越",
          "slug": null
        },
        {
          "display": "新潟県央工",
          "score": 0,
          "won": false,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "佐渡",
          "score": 8,
          "won": true,
          "name": "佐渡高校",
          "slug": "sado"
        },
        {
          "display": "巻総合",
          "score": 7,
          "won": false,
          "name": "巻総合高校",
          "slug": "makisogo"
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "佐渡総合",
          "score": 2,
          "won": false,
          "name": "佐渡総合高校",
          "slug": "sadosogo"
        },
        {
          "display": "新発田南",
          "score": 15,
          "won": true,
          "name": "新発田南高校",
          "slug": "shibataminami"
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "十日町総合",
          "score": 8,
          "won": false,
          "name": "十日町総合高校",
          "slug": "tokamachisogo"
        },
        {
          "display": "新発田中央",
          "score": 15,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-16",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "3回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "加茂",
          "score": 0,
          "won": false,
          "name": "加茂高校",
          "slug": "kamo"
        },
        {
          "display": "新潟工",
          "score": 10,
          "won": true,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "4回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "日本文理",
          "score": 8,
          "won": true,
          "name": "日本文理",
          "slug": null
        },
        {
          "display": "六日町",
          "score": 0,
          "won": false,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "4回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "五泉",
          "score": 3,
          "won": false,
          "name": "五泉高校",
          "slug": "gosen"
        },
        {
          "display": "東京学館新潟",
          "score": 4,
          "won": true,
          "name": "東京学館新潟",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "4回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "帝京長岡",
          "score": 16,
          "won": true,
          "name": "帝京長岡",
          "slug": null
        },
        {
          "display": "新発田南",
          "score": 0,
          "won": false,
          "name": "新発田南高校",
          "slug": "shibataminami"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "4回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "新潟南",
          "score": 0,
          "won": false,
          "name": "新潟南高校",
          "slug": "niigataminami"
        },
        {
          "display": "新潟産大附",
          "score": 7,
          "won": true,
          "name": "新潟産大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "4回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "新潟工",
          "score": 3,
          "won": false,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        },
        {
          "display": "新発田中央",
          "score": 4,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権新潟大会",
      "round": "4回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "佐渡",
          "score": 3,
          "won": false,
          "name": "佐渡高校",
          "slug": "sado"
        },
        {
          "display": "北越",
          "score": 8,
          "won": true,
          "name": "北越",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-11",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "加茂農林",
          "score": 0,
          "won": false,
          "name": "加茂農林高校",
          "slug": "kamonorin"
        },
        {
          "display": "新潟西",
          "score": 16,
          "won": true,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        }
      ]
    },
    {
      "date": "2025-09-11",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "敬和学園",
          "score": 8,
          "won": false,
          "name": "敬和学園",
          "slug": null
        },
        {
          "display": "加茂",
          "score": 17,
          "won": true,
          "name": "加茂高校",
          "slug": "kamo"
        }
      ]
    },
    {
      "date": "2025-09-11",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "万代",
          "score": 3,
          "won": false,
          "name": "万代高校",
          "slug": "bandai"
        },
        {
          "display": "新潟第一",
          "score": 5,
          "won": true,
          "name": "新潟第一",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-11",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "新潟江南",
          "score": 1,
          "won": false,
          "name": "新潟江南高校",
          "slug": "niigatakonan"
        },
        {
          "display": "新潟明訓",
          "score": 12,
          "won": true,
          "name": "新潟明訓",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-11",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "上越",
          "score": 0,
          "won": false,
          "name": "上越",
          "slug": null
        },
        {
          "display": "六日町",
          "score": 4,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2025-09-11",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "新潟県央工",
          "score": 11,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        },
        {
          "display": "高田農",
          "score": 0,
          "won": false,
          "name": "高田農業高校",
          "slug": "takadanogyo"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟商",
          "score": 1,
          "won": false,
          "name": "新潟商業高校",
          "slug": "niigatashogyo"
        },
        {
          "display": "新発田農",
          "score": 4,
          "won": true,
          "name": "新発田農業高校",
          "slug": "shibatanogyo"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "村上桜ケ丘",
          "score": 2,
          "won": false,
          "name": "村上桜ヶ丘高校",
          "slug": "murakamisakuragaoka"
        },
        {
          "display": "加茂暁星",
          "score": 9,
          "won": true,
          "name": "加茂暁星",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "北村上中条",
          "score": 1,
          "won": false,
          "name": "北村上中条",
          "slug": null
        },
        {
          "display": "新潟工",
          "score": 11,
          "won": true,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "佐渡",
          "score": 3,
          "won": false,
          "name": "佐渡高校",
          "slug": "sado"
        },
        {
          "display": "日本文理",
          "score": 13,
          "won": true,
          "name": "日本文理",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "佐渡総合",
          "score": 2,
          "won": false,
          "name": "佐渡総合高校",
          "slug": "sadosogo"
        },
        {
          "display": "新潟南",
          "score": 9,
          "won": true,
          "name": "新潟南高校",
          "slug": "niigataminami"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "五泉",
          "score": 1,
          "won": false,
          "name": "五泉高校",
          "slug": "gosen"
        },
        {
          "display": "開志学園",
          "score": 6,
          "won": true,
          "name": "開志学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡大手",
          "score": 13,
          "won": true,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "三条東",
          "score": 0,
          "won": false,
          "name": "三条東高校",
          "slug": "sanjohigashi"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡向陵",
          "score": 5,
          "won": true,
          "name": "長岡向陵高校",
          "slug": "nagaokakoryo"
        },
        {
          "display": "十総・塩沢",
          "score": 4,
          "won": false,
          "name": "十総・塩沢",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "上越総合技術",
          "score": 13,
          "won": true,
          "name": "上越総合技術高校",
          "slug": "joetsusogogijutsu"
        },
        {
          "display": "小出",
          "score": 1,
          "won": false,
          "name": "小出高校",
          "slug": "koide"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "高田北城",
          "score": 13,
          "won": true,
          "name": "高田北城高校",
          "slug": "takadahokujo"
        },
        {
          "display": "柏崎工",
          "score": 0,
          "won": false,
          "name": "柏崎工業高校",
          "slug": "kashiwazakikogyo"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "海洋",
          "score": 0,
          "won": false,
          "name": "海洋高校",
          "slug": "niigata-kaiyo"
        },
        {
          "display": "十日町",
          "score": 10,
          "won": true,
          "name": "十日町高校",
          "slug": "tokamachi"
        }
      ]
    },
    {
      "date": "2025-09-12",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡工",
          "score": 1,
          "won": false,
          "name": "長岡工業高校",
          "slug": "nagaokakogyo"
        },
        {
          "display": "糸魚川",
          "score": 7,
          "won": true,
          "name": "糸魚川高校",
          "slug": "itoigawa"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新発田南",
          "score": 7,
          "won": false,
          "name": "新発田南高校",
          "slug": "shibataminami"
        },
        {
          "display": "新発田中央",
          "score": 9,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新津工",
          "score": 1,
          "won": false,
          "name": "新津工業高校",
          "slug": "niitsukogyo"
        },
        {
          "display": "新発田",
          "score": 8,
          "won": true,
          "name": "新発田高校",
          "slug": "niigata-shibata"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新津",
          "score": 9,
          "won": true,
          "name": "新津高校",
          "slug": "niitsu"
        },
        {
          "display": "東向南商",
          "score": 0,
          "won": false,
          "name": "東向南商",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "新潟",
          "score": 6,
          "won": true,
          "name": "新潟高校",
          "slug": "niigata"
        },
        {
          "display": "巻",
          "score": 3,
          "won": false,
          "name": "巻高校",
          "slug": "maki"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "巻総合",
          "score": 1,
          "won": false,
          "name": "巻総合高校",
          "slug": "makisogo"
        },
        {
          "display": "東京学館新潟",
          "score": 7,
          "won": true,
          "name": "東京学館新潟",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡",
          "score": 6,
          "won": false,
          "name": "長岡高校",
          "slug": "nagaoka"
        },
        {
          "display": "三条",
          "score": 7,
          "won": true,
          "name": "三条高校",
          "slug": "sanjo"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "小千谷",
          "score": 0,
          "won": false,
          "name": "小千谷高校",
          "slug": "ojiya"
        },
        {
          "display": "帝京長岡",
          "score": 10,
          "won": true,
          "name": "帝京長岡",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "関根学園",
          "score": 4,
          "won": true,
          "name": "関根学園",
          "slug": null
        },
        {
          "display": "柏崎",
          "score": 1,
          "won": false,
          "name": "柏崎高校",
          "slug": "kashiwazaki"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡工",
          "score": 1,
          "won": false,
          "name": "長岡工業高校",
          "slug": "nagaokakogyo"
        },
        {
          "display": "糸魚川",
          "score": 8,
          "won": true,
          "name": "糸魚川高校",
          "slug": "itoigawa"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "6校連合",
          "score": 1,
          "won": false,
          "name": "6校連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "高田",
          "score": 4,
          "won": true,
          "name": "高田高校",
          "slug": "niigata-takata"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "1回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "常総新商嶺",
          "score": 1,
          "won": false,
          "name": "常総新商嶺",
          "slug": null
        },
        {
          "display": "長岡商",
          "score": 7,
          "won": true,
          "name": "長岡商業高校",
          "slug": "nagaokashogyo"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新発田農",
          "score": 4,
          "won": false,
          "name": "新発田農業高校",
          "slug": "shibatanogyo"
        },
        {
          "display": "開志学園",
          "score": 10,
          "won": true,
          "name": "開志学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "加茂",
          "score": 4,
          "won": false,
          "name": "加茂高校",
          "slug": "kamo"
        },
        {
          "display": "新潟西",
          "score": 6,
          "won": true,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡高専",
          "score": 0,
          "won": false,
          "name": "長岡工業高専",
          "slug": "nagaokakogyo-kosen"
        },
        {
          "display": "新潟県央工",
          "score": 18,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "小千谷西",
          "score": 0,
          "won": false,
          "name": "小千谷西高校",
          "slug": "ojiyanishi"
        },
        {
          "display": "六日町",
          "score": 13,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡向陵",
          "score": 0,
          "won": false,
          "name": "長岡向陵高校",
          "slug": "nagaokakoryo"
        },
        {
          "display": "十日町",
          "score": 5,
          "won": true,
          "name": "十日町高校",
          "slug": "tokamachi"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新津",
          "score": 0,
          "won": false,
          "name": "新津高校",
          "slug": "niitsu"
        },
        {
          "display": "新発田中央",
          "score": 10,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新潟",
          "score": 5,
          "won": false,
          "name": "新潟高校",
          "slug": "niigata"
        },
        {
          "display": "新発田",
          "score": 6,
          "won": true,
          "name": "新発田高校",
          "slug": "niigata-shibata"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "新潟工",
          "score": 5,
          "won": true,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        },
        {
          "display": "新潟南",
          "score": 4,
          "won": false,
          "name": "新潟南高校",
          "slug": "niigataminami"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "中越",
          "score": 7,
          "won": true,
          "name": "中越",
          "slug": null
        },
        {
          "display": "三条",
          "score": 0,
          "won": false,
          "name": "三条高校",
          "slug": "sanjo"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡大手",
          "score": 7,
          "won": true,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "上越総合技術",
          "score": 5,
          "won": false,
          "name": "上越総合技術高校",
          "slug": "joetsusogogijutsu"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "高田北城",
          "score": 4,
          "won": true,
          "name": "高田北城高校",
          "slug": "takadahokujo"
        },
        {
          "display": "糸魚川",
          "score": 2,
          "won": false,
          "name": "糸魚川高校",
          "slug": "itoigawa"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "2回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡商",
          "score": 9,
          "won": false,
          "name": "長岡商業高校",
          "slug": "nagaokashogyo"
        },
        {
          "display": "高田",
          "score": 12,
          "won": true,
          "name": "高田高校",
          "slug": "niigata-takata"
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "高田",
          "score": 0,
          "won": false,
          "name": "高田高校",
          "slug": "niigata-takata"
        },
        {
          "display": "新発田中央",
          "score": 7,
          "won": true,
          "name": "新発田中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "五十公野",
      "teams": [
        {
          "display": "新発田",
          "score": 0,
          "won": false,
          "name": "新発田高校",
          "slug": "niigata-shibata"
        },
        {
          "display": "中越",
          "score": 10,
          "won": true,
          "name": "中越",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "新潟明訓",
          "score": 3,
          "won": false,
          "name": "新潟明訓",
          "slug": null
        },
        {
          "display": "六日町",
          "score": 5,
          "won": true,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "三条パール",
      "teams": [
        {
          "display": "開志学園",
          "score": 2,
          "won": true,
          "name": "開志学園",
          "slug": null
        },
        {
          "display": "十日町",
          "score": 1,
          "won": false,
          "name": "十日町高校",
          "slug": "tokamachi"
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "新潟西",
          "score": 0,
          "won": false,
          "name": "新潟西高校",
          "slug": "niigatanishi"
        },
        {
          "display": "新潟県央工",
          "score": 8,
          "won": true,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "悠久山",
      "teams": [
        {
          "display": "高田北城",
          "score": 0,
          "won": false,
          "name": "高田北城高校",
          "slug": "takadahokujo"
        },
        {
          "display": "日本文理",
          "score": 10,
          "won": true,
          "name": "日本文理",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-19",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "3回戦",
      "venue": "佐藤池",
      "teams": [
        {
          "display": "長岡大手",
          "score": 6,
          "won": true,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "新潟工",
          "score": 0,
          "won": false,
          "name": "新潟工業高校",
          "slug": "niigatakogyo"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "準々決勝",
      "venue": "三条パール",
      "teams": [
        {
          "display": "開志学園",
          "score": 6,
          "won": true,
          "name": "開志学園",
          "slug": null
        },
        {
          "display": "六日町",
          "score": 1,
          "won": false,
          "name": "六日町高校",
          "slug": "muikamachi"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "準々決勝",
      "venue": "悠久山",
      "teams": [
        {
          "display": "新潟県央工",
          "score": 0,
          "won": false,
          "name": "新潟県央工業高校",
          "slug": "niigatakenokogyo"
        },
        {
          "display": "日本文理",
          "score": 1,
          "won": true,
          "name": "日本文理",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第155回北信越地区高等学校野球新潟県大会（令和8年度秋季）",
      "round": "準々決勝",
      "venue": "悠久山",
      "teams": [
        {
          "display": "長岡大手",
          "score": 1,
          "won": false,
          "name": "長岡大手高校",
          "slug": "nagaokaote"
        },
        {
          "display": "帝京長岡",
          "score": 8,
          "won": true,
          "name": "帝京長岡",
          "slug": null
        }
      ]
    }
  ]
};
