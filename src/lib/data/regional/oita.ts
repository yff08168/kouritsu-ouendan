// このファイルは scripts/build-regional-results.mjs が生成する。直接編集しない。
// 出典: 大分県高等学校野球連盟（https://www.oita-kouyaren.com/）

import type { RegionalDistrict } from "@/lib/regional-results";

export const REGIONAL_OITA: RegionalDistrict = {
  "slug": "oita",
  "district": "大分",
  "sourceName": "大分県高等学校野球連盟",
  "sourceUrl": "https://www.oita-kouyaren.com/",
  "games": [
    {
      "date": "2026-05-16",
      "season": "spring",
      "tournament": "第149回大分県高等学校野球選手権大会",
      "round": "準々決勝",
      "venue": null,
      "teams": [
        {
          "display": "柳ヶ浦",
          "score": 5,
          "won": false,
          "name": "柳ヶ浦",
          "slug": null
        },
        {
          "display": "大分商業",
          "score": 6,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        }
      ]
    },
    {
      "date": "2026-05-17",
      "season": "spring",
      "tournament": "第149回大分県高等学校野球選手権大会",
      "round": "準々決勝",
      "venue": null,
      "teams": [
        {
          "display": "楊志館",
          "score": 10,
          "won": true,
          "name": "楊志館",
          "slug": null
        },
        {
          "display": "佐伯鶴城",
          "score": 0,
          "won": false,
          "name": "佐伯鶴城高校",
          "slug": "saikikakujo"
        }
      ]
    },
    {
      "date": "2026-05-17",
      "season": "spring",
      "tournament": "第149回大分県高等学校野球選手権大会",
      "round": "準々決勝",
      "venue": null,
      "teams": [
        {
          "display": "大分舞鶴",
          "score": 6,
          "won": true,
          "name": "大分舞鶴高校",
          "slug": "oitamaizuru"
        },
        {
          "display": "大分上野丘",
          "score": 4,
          "won": false,
          "name": "大分上野丘高校",
          "slug": "oitauenogaoka"
        }
      ]
    },
    {
      "date": "2026-05-23",
      "season": "spring",
      "tournament": "第149回大分県高等学校野球選手権大会",
      "round": "準決勝",
      "venue": null,
      "teams": [
        {
          "display": "明豊",
          "score": 2,
          "won": false,
          "name": "明豊",
          "slug": null
        },
        {
          "display": "大分商業",
          "score": 4,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        }
      ]
    },
    {
      "date": "2026-05-23",
      "season": "spring",
      "tournament": "第149回大分県高等学校野球選手権大会",
      "round": "準決勝",
      "venue": null,
      "teams": [
        {
          "display": "楊志館",
          "score": 8,
          "won": true,
          "name": "楊志館",
          "slug": null
        },
        {
          "display": "大分舞鶴",
          "score": 1,
          "won": false,
          "name": "大分舞鶴高校",
          "slug": "oitamaizuru"
        }
      ]
    },
    {
      "date": "2026-05-24",
      "season": "spring",
      "tournament": "第149回大分県高等学校野球選手権大会",
      "round": "決勝",
      "venue": null,
      "teams": [
        {
          "display": "大分商業",
          "score": 9,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        },
        {
          "display": "楊志館",
          "score": 0,
          "won": false,
          "name": "楊志館",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "高田",
          "score": 10,
          "won": true,
          "name": "高田高校",
          "slug": "oita-takata"
        },
        {
          "display": "中津北",
          "score": 0,
          "won": false,
          "name": "中津北高校",
          "slug": "nakatsukita"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分高専",
          "score": 5,
          "won": true,
          "name": "大分工業高専",
          "slug": "oitakogyo-kosen"
        },
        {
          "display": "玖珠美山",
          "score": 0,
          "won": false,
          "name": "玖珠美山高校",
          "slug": "kusumiyama"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "国東",
          "score": 7,
          "won": true,
          "name": "国東高校",
          "slug": "kunisaki"
        },
        {
          "display": "佐伯豊南",
          "score": 6,
          "won": false,
          "name": "佐伯豊南高校",
          "slug": "saikihonan"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "佐伯鶴城",
          "score": 13,
          "won": true,
          "name": "佐伯鶴城高校",
          "slug": "saikikakujo"
        },
        {
          "display": "臼杵",
          "score": 0,
          "won": false,
          "name": "臼杵高校",
          "slug": "usuki"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "柳ヶ浦",
          "score": 2,
          "won": true,
          "name": "柳ヶ浦",
          "slug": null
        },
        {
          "display": "日田林工",
          "score": 1,
          "won": false,
          "name": "日田林工高校",
          "slug": "hitarinko"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "宇佐産・由布",
          "score": 3,
          "won": false,
          "name": "宇佐産・由布",
          "slug": null,
          "combined": true
        },
        {
          "display": "宇佐",
          "score": 7,
          "won": true,
          "name": "宇佐高校",
          "slug": "usa"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "日本文理大附",
          "score": 2,
          "won": true,
          "name": "日本文理大附",
          "slug": null
        },
        {
          "display": "大分南",
          "score": 1,
          "won": false,
          "name": "大分南高校",
          "slug": "oitaminami"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "三重総合",
          "score": 7,
          "won": true,
          "name": "三重総合高校",
          "slug": "miesogo"
        },
        {
          "display": "大分西",
          "score": 5,
          "won": false,
          "name": "大分西高校",
          "slug": "oitanishi"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "1回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分国際情報",
          "score": 6,
          "won": false,
          "name": "大分国際情報",
          "slug": null
        },
        {
          "display": "大分工業",
          "score": 9,
          "won": true,
          "name": "大分工業高校",
          "slug": "oitakogyo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "明豊",
          "score": 5,
          "won": true,
          "name": "明豊",
          "slug": null
        },
        {
          "display": "高田",
          "score": 0,
          "won": false,
          "name": "高田高校",
          "slug": "oita-takata"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分雄城台",
          "score": 10,
          "won": true,
          "name": "大分雄城台高校",
          "slug": "oitaoginodai"
        },
        {
          "display": "中津南",
          "score": 5,
          "won": false,
          "name": "中津南高校",
          "slug": "nakatsuminami"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "藤蔭",
          "score": 11,
          "won": true,
          "name": "藤蔭",
          "slug": null
        },
        {
          "display": "大分高専",
          "score": 1,
          "won": false,
          "name": "大分工業高専",
          "slug": "oitakogyo-kosen"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "別府鶴見丘",
          "score": 1,
          "won": false,
          "name": "別府鶴見丘高校",
          "slug": "bepputsurumigaoka"
        },
        {
          "display": "情報科学",
          "score": 5,
          "won": true,
          "name": "情報科学高校",
          "slug": "oita-johokagaku"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分商業",
          "score": 9,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        },
        {
          "display": "国東",
          "score": 1,
          "won": false,
          "name": "国東高校",
          "slug": "kunisaki"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "中津東",
          "score": 14,
          "won": true,
          "name": "中津東高校",
          "slug": "nakatsuhigashi"
        },
        {
          "display": "大分東",
          "score": 0,
          "won": false,
          "name": "大分東高校",
          "slug": "oitahigashi"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "楊志館",
          "score": 2,
          "won": false,
          "name": "楊志館",
          "slug": null
        },
        {
          "display": "佐伯鶴城",
          "score": 5,
          "won": true,
          "name": "佐伯鶴城高校",
          "slug": "saikikakujo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "別府翔青",
          "score": 0,
          "won": false,
          "name": "別府翔青高校",
          "slug": "beppushosei"
        },
        {
          "display": "柳ヶ浦",
          "score": 2,
          "won": true,
          "name": "柳ヶ浦",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分上野丘",
          "score": 6,
          "won": true,
          "name": "大分上野丘高校",
          "slug": "oitauenogaoka"
        },
        {
          "display": "宇佐",
          "score": 0,
          "won": false,
          "name": "宇佐高校",
          "slug": "usa"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "竹田",
          "score": 4,
          "won": true,
          "name": "竹田高校",
          "slug": "taketa"
        },
        {
          "display": "大分東明",
          "score": 3,
          "won": false,
          "name": "大分東明",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "津久見",
          "score": 5,
          "won": true,
          "name": "津久見高校",
          "slug": "tsukumi"
        },
        {
          "display": "日本文理大附",
          "score": 1,
          "won": false,
          "name": "日本文理大附",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "鶴崎工業",
          "score": 9,
          "won": true,
          "name": "鶴崎工業高校",
          "slug": "tsurusakikogyo"
        },
        {
          "display": "日田",
          "score": 1,
          "won": false,
          "name": "日田高校",
          "slug": "hita"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分舞鶴",
          "score": 8,
          "won": true,
          "name": "大分舞鶴高校",
          "slug": "oitamaizuru"
        },
        {
          "display": "三重総合",
          "score": 1,
          "won": false,
          "name": "三重総合高校",
          "slug": "miesogo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分豊府",
          "score": 4,
          "won": true,
          "name": "大分豊府高校",
          "slug": "oitahofu"
        },
        {
          "display": "大分鶴崎",
          "score": 3,
          "won": false,
          "name": "大分鶴崎高校",
          "slug": "oitatsurusaki"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "杵築",
          "score": 1,
          "won": false,
          "name": "杵築高校",
          "slug": "kitsuki"
        },
        {
          "display": "大分工業",
          "score": 11,
          "won": true,
          "name": "大分工業高校",
          "slug": "oitakogyo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "2回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分",
          "score": 8,
          "won": true,
          "name": "大分",
          "slug": null
        },
        {
          "display": "日出総合",
          "score": 1,
          "won": false,
          "name": "日出総合高校",
          "slug": "hijisogo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "明豊",
          "score": 7,
          "won": true,
          "name": "明豊",
          "slug": null
        },
        {
          "display": "大分雄城台",
          "score": 0,
          "won": false,
          "name": "大分雄城台高校",
          "slug": "oitaoginodai"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "藤蔭",
          "score": 13,
          "won": true,
          "name": "藤蔭",
          "slug": null
        },
        {
          "display": "情報科学",
          "score": 3,
          "won": false,
          "name": "情報科学高校",
          "slug": "oita-johokagaku"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分商業",
          "score": 8,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        },
        {
          "display": "中津東",
          "score": 4,
          "won": false,
          "name": "中津東高校",
          "slug": "nakatsuhigashi"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "佐伯鶴城",
          "score": 7,
          "won": true,
          "name": "佐伯鶴城高校",
          "slug": "saikikakujo"
        },
        {
          "display": "柳ヶ浦",
          "score": 6,
          "won": false,
          "name": "柳ヶ浦",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分上野丘",
          "score": 12,
          "won": true,
          "name": "大分上野丘高校",
          "slug": "oitauenogaoka"
        },
        {
          "display": "竹田",
          "score": 2,
          "won": false,
          "name": "竹田高校",
          "slug": "taketa"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "津久見",
          "score": 6,
          "won": true,
          "name": "津久見高校",
          "slug": "tsukumi"
        },
        {
          "display": "鶴崎工業",
          "score": 5,
          "won": false,
          "name": "鶴崎工業高校",
          "slug": "tsurusakikogyo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分舞鶴",
          "score": 8,
          "won": true,
          "name": "大分舞鶴高校",
          "slug": "oitamaizuru"
        },
        {
          "display": "大分豊府",
          "score": 4,
          "won": false,
          "name": "大分豊府高校",
          "slug": "oitahofu"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "3回戦",
      "venue": null,
      "teams": [
        {
          "display": "大分工業",
          "score": 4,
          "won": false,
          "name": "大分工業高校",
          "slug": "oitakogyo"
        },
        {
          "display": "大分",
          "score": 6,
          "won": true,
          "name": "大分",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "準々決勝",
      "venue": null,
      "teams": [
        {
          "display": "大分商業",
          "score": 5,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        },
        {
          "display": "佐伯鶴城",
          "score": 3,
          "won": false,
          "name": "佐伯鶴城高校",
          "slug": "saikikakujo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "準々決勝",
      "venue": null,
      "teams": [
        {
          "display": "大分上野丘",
          "score": 3,
          "won": false,
          "name": "大分上野丘高校",
          "slug": "oitauenogaoka"
        },
        {
          "display": "津久見",
          "score": 10,
          "won": true,
          "name": "津久見高校",
          "slug": "tsukumi"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "準々決勝",
      "venue": null,
      "teams": [
        {
          "display": "大分舞鶴",
          "score": 7,
          "won": true,
          "name": "大分舞鶴高校",
          "slug": "oitamaizuru"
        },
        {
          "display": "大分",
          "score": 4,
          "won": false,
          "name": "大分",
          "slug": null
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "準決勝",
      "venue": null,
      "teams": [
        {
          "display": "明豊",
          "score": 3,
          "won": false,
          "name": "明豊",
          "slug": null
        },
        {
          "display": "大分商業",
          "score": 4,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "準決勝",
      "venue": null,
      "teams": [
        {
          "display": "津久見",
          "score": 6,
          "won": true,
          "name": "津久見高校",
          "slug": "tsukumi"
        },
        {
          "display": "大分舞鶴",
          "score": 4,
          "won": false,
          "name": "大分舞鶴高校",
          "slug": "oitamaizuru"
        }
      ]
    },
    {
      "date": null,
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権大分大会",
      "round": "決勝",
      "venue": null,
      "teams": [
        {
          "display": "大分商業",
          "score": 6,
          "won": true,
          "name": "大分商業高校",
          "slug": "oitashogyo"
        },
        {
          "display": "津久見",
          "score": 2,
          "won": false,
          "name": "津久見高校",
          "slug": "tsukumi"
        }
      ]
    }
  ]
};
