// このファイルは scripts/build-regional-results.mjs が生成する。直接編集しない。
// 出典: 栃木県高校野球連盟（https://www.tochigi-koyaren.net/）

import type { RegionalDistrict } from "@/lib/regional-results";

export const REGIONAL_TOCHIGI: RegionalDistrict = {
  "slug": "tochigi",
  "district": "栃木",
  "sourceName": "栃木県高校野球連盟",
  "sourceUrl": "https://www.tochigi-koyaren.net/",
  "games": [
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮北",
          "score": 2,
          "won": false,
          "name": "宇都宮北高校",
          "slug": "utsunomiyakita"
        },
        {
          "display": "那須清峰",
          "score": 9,
          "won": true,
          "name": "那須清峰高校",
          "slug": "nasuseiho"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "小山",
          "score": 9,
          "won": false,
          "name": "小山高校",
          "slug": "oyama"
        },
        {
          "display": "宇都宮中央",
          "score": 10,
          "won": true,
          "name": "宇都宮中央高校",
          "slug": "utsunomiyachuo"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮商",
          "score": 7,
          "won": false,
          "name": "宇都宮商業高校",
          "slug": "utsunomiyashogyo"
        },
        {
          "display": "宇都宮東",
          "score": 8,
          "won": true,
          "name": "宇都宮東高校",
          "slug": "utsunomiyahigashi"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "小山西",
          "score": 13,
          "won": true,
          "name": "小山西高校",
          "slug": "oyamanishi"
        },
        {
          "display": "茂木",
          "score": 4,
          "won": false,
          "name": "茂木高校",
          "slug": "motegi"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "連合",
          "score": 0,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "宇都宮南",
          "score": 7,
          "won": true,
          "name": "宇都宮南高校",
          "slug": "utsunomiyaminami"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "小山南",
          "score": 8,
          "won": true,
          "name": "小山南高校",
          "slug": "oyamaminami"
        },
        {
          "display": "真岡工",
          "score": 5,
          "won": false,
          "name": "真岡工業高校",
          "slug": "mokakogyo"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "宇都宮",
          "score": 11,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        },
        {
          "display": "鹿沼南",
          "score": 1,
          "won": false,
          "name": "鹿沼南高校",
          "slug": "kanumaminami"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "白鷗大足利",
          "score": 12,
          "won": true,
          "name": "白鷗大足利",
          "slug": null
        },
        {
          "display": "宇短大附",
          "score": 0,
          "won": false,
          "name": "宇短大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "石川スポーツグラウンドくろいそ",
      "teams": [
        {
          "display": "那須拓陽",
          "score": 17,
          "won": true,
          "name": "那須拓陽高校",
          "slug": "nasutakuyo"
        },
        {
          "display": "さくら清修",
          "score": 0,
          "won": false,
          "name": "さくら清修高校",
          "slug": "sakuraseishu"
        }
      ]
    },
    {
      "date": "2026-04-11",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "石川スポーツグラウンドくろいそ",
      "teams": [
        {
          "display": "黒磯",
          "score": 14,
          "won": true,
          "name": "黒磯高校",
          "slug": "kuroiso"
        },
        {
          "display": "小山北桜",
          "score": 3,
          "won": false,
          "name": "小山北桜高校",
          "slug": "oyamahokuo"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "足利大附",
          "score": 2,
          "won": false,
          "name": "足利大附",
          "slug": null
        },
        {
          "display": "栃木商",
          "score": 6,
          "won": true,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "矢板東",
          "score": 0,
          "won": false,
          "name": "矢板東高校",
          "slug": "yaitahigashi"
        },
        {
          "display": "青藍泰斗",
          "score": 24,
          "won": true,
          "name": "青藍泰斗",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮白楊",
          "score": 3,
          "won": false,
          "name": "宇都宮白楊高校",
          "slug": "utsunomiyahakuyo"
        },
        {
          "display": "真岡",
          "score": 10,
          "won": true,
          "name": "真岡高校",
          "slug": "moka"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "佐野",
          "score": 0,
          "won": false,
          "name": "佐野高校",
          "slug": "sano"
        },
        {
          "display": "栃木",
          "score": 15,
          "won": true,
          "name": "栃木高校",
          "slug": "tochigi"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "連合",
          "score": 9,
          "won": true,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "鹿沼商工",
          "score": 5,
          "won": false,
          "name": "鹿沼商工高校",
          "slug": "kanumashoko"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "足利",
          "score": 18,
          "won": true,
          "name": "足利高校",
          "slug": "ashikaga"
        },
        {
          "display": "足利工",
          "score": 16,
          "won": false,
          "name": "足利工業高校",
          "slug": "ashikagakogyo"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "連合",
          "score": 16,
          "won": true,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "高根沢",
          "score": 11,
          "won": false,
          "name": "高根沢高校",
          "slug": "takanezawa"
        }
      ]
    },
    {
      "date": "2026-04-12",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "小山高専",
          "score": 9,
          "won": true,
          "name": "小山工業高専",
          "slug": "oyamakogyo"
        },
        {
          "display": "上三川",
          "score": 8,
          "won": false,
          "name": "上三川高校",
          "slug": "kaminokawa"
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮南",
          "score": 0,
          "won": false,
          "name": "宇都宮南高校",
          "slug": "utsunomiyaminami"
        },
        {
          "display": "國學院栃木",
          "score": 0,
          "won": false,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮工",
          "score": 1,
          "won": false,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        },
        {
          "display": "白鷗大足利",
          "score": 3,
          "won": true,
          "name": "白鷗大足利",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "石橋",
          "score": 11,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        },
        {
          "display": "那須拓陽",
          "score": 3,
          "won": false,
          "name": "那須拓陽高校",
          "slug": "nasutakuyo"
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 7,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "小山南",
          "score": 0,
          "won": false,
          "name": "小山南高校",
          "slug": "oyamaminami"
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "宇都宮",
          "score": 9,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        },
        {
          "display": "宇都宮東",
          "score": 2,
          "won": false,
          "name": "宇都宮東高校",
          "slug": "utsunomiyahigashi"
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "小山西",
          "score": 5,
          "won": true,
          "name": "小山西高校",
          "slug": "oyamanishi"
        },
        {
          "display": "連合",
          "score": 2,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "矢板中央",
          "score": 16,
          "won": true,
          "name": "矢板中央",
          "slug": null
        },
        {
          "display": "那須清峰",
          "score": 2,
          "won": false,
          "name": "那須清峰高校",
          "slug": "nasuseiho"
        }
      ]
    },
    {
      "date": "2026-04-18",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "連合",
          "score": 7,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "宇都宮中央",
          "score": 17,
          "won": true,
          "name": "宇都宮中央高校",
          "slug": "utsunomiyachuo"
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "連合",
          "score": 5,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "幸福学園",
          "score": 7,
          "won": true,
          "name": "幸福学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "足利",
          "score": 1,
          "won": false,
          "name": "足利高校",
          "slug": "ashikaga"
        },
        {
          "display": "栃木工",
          "score": 9,
          "won": true,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "黒磯",
          "score": 1,
          "won": false,
          "name": "黒磯高校",
          "slug": "kuroiso"
        },
        {
          "display": "作新学院",
          "score": 9,
          "won": true,
          "name": "作新学院",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "小山高専",
          "score": 0,
          "won": false,
          "name": "小山工業高専",
          "slug": "oyamakogyo"
        },
        {
          "display": "佐野日大",
          "score": 18,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "栃木商",
          "score": 6,
          "won": true,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        },
        {
          "display": "真岡",
          "score": 5,
          "won": false,
          "name": "真岡高校",
          "slug": "moka"
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "鹿沼",
          "score": 5,
          "won": true,
          "name": "鹿沼高校",
          "slug": "kanuma"
        },
        {
          "display": "栃木",
          "score": 4,
          "won": false,
          "name": "栃木高校",
          "slug": "tochigi"
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "大田原",
          "score": 5,
          "won": false,
          "name": "大田原高校",
          "slug": "otawara"
        },
        {
          "display": "青藍泰斗",
          "score": 12,
          "won": true,
          "name": "青藍泰斗",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-19",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "烏山",
          "score": 7,
          "won": true,
          "name": "烏山高校",
          "slug": "karasuyama"
        },
        {
          "display": "連合",
          "score": 0,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "石橋",
          "score": 9,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        },
        {
          "display": "宇都宮",
          "score": 2,
          "won": false,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "小山西",
          "score": 3,
          "won": false,
          "name": "小山西高校",
          "slug": "oyamanishi"
        },
        {
          "display": "文星芸大附",
          "score": 10,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "矢板中央",
          "score": 2,
          "won": false,
          "name": "矢板中央",
          "slug": null
        },
        {
          "display": "國學院栃木",
          "score": 7,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-25",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "白鷗大足利",
          "score": 10,
          "won": true,
          "name": "白鷗大足利",
          "slug": null
        },
        {
          "display": "宇都宮中央",
          "score": 1,
          "won": false,
          "name": "宇都宮中央高校",
          "slug": "utsunomiyachuo"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "栃木商",
          "score": 4,
          "won": false,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        },
        {
          "display": "作新学院",
          "score": 14,
          "won": true,
          "name": "作新学院",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "佐野日大",
          "score": 2,
          "won": true,
          "name": "佐野日大",
          "slug": null
        },
        {
          "display": "鹿沼",
          "score": 1,
          "won": false,
          "name": "鹿沼高校",
          "slug": "kanuma"
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "青藍泰斗",
          "score": 9,
          "won": true,
          "name": "青藍泰斗",
          "slug": null
        },
        {
          "display": "幸福学園",
          "score": 8,
          "won": false,
          "name": "幸福学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-26",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "烏山",
          "score": 0,
          "won": false,
          "name": "烏山高校",
          "slug": "karasuyama"
        },
        {
          "display": "栃木工",
          "score": 10,
          "won": true,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        }
      ]
    },
    {
      "date": "2026-04-29",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "準々決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 5,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "白鷗大足利",
          "score": 0,
          "won": false,
          "name": "白鷗大足利",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-29",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "準々決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "栃木工",
          "score": 1,
          "won": false,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        },
        {
          "display": "佐野日大",
          "score": 4,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-29",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "準々決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "石橋",
          "score": 7,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        },
        {
          "display": "國學院栃木",
          "score": 6,
          "won": false,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-04-29",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "準々決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "作新学院",
          "score": 5,
          "won": false,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "青藍泰斗",
          "score": 8,
          "won": true,
          "name": "青藍泰斗",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "準決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 10,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "石橋",
          "score": 1,
          "won": false,
          "name": "石橋高校",
          "slug": "ishibashi"
        }
      ]
    },
    {
      "date": "2026-05-02",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "準決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "青藍泰斗",
          "score": 5,
          "won": false,
          "name": "青藍泰斗",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 7,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-05-03",
      "season": "spring",
      "tournament": "第79回春季栃木県高等学校野球大会試合結果",
      "round": "決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 9,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 4,
          "won": false,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-09",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "鹿沼商工",
          "score": 8,
          "won": true,
          "name": "鹿沼商工高校",
          "slug": "kanumashoko"
        },
        {
          "display": "さくら清修",
          "score": 1,
          "won": false,
          "name": "さくら清修高校",
          "slug": "sakuraseishu"
        }
      ]
    },
    {
      "date": "2026-07-09",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "鹿沼",
          "score": 4,
          "won": true,
          "name": "鹿沼高校",
          "slug": "kanuma"
        },
        {
          "display": "真岡",
          "score": 3,
          "won": false,
          "name": "真岡高校",
          "slug": "moka"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇短大附",
          "score": 15,
          "won": true,
          "name": "宇短大附",
          "slug": null
        },
        {
          "display": "小山北桜",
          "score": 3,
          "won": false,
          "name": "小山北桜高校",
          "slug": "oyamahokuo"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "連合",
          "score": 2,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "足利",
          "score": 7,
          "won": true,
          "name": "足利高校",
          "slug": "ashikaga"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "真岡北陵",
          "score": 0,
          "won": false,
          "name": "真岡北陵高校",
          "slug": "mokahokuryo"
        },
        {
          "display": "足利工",
          "score": 21,
          "won": true,
          "name": "足利工業高校",
          "slug": "ashikagakogyo"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "那須清峰",
          "score": 17,
          "won": true,
          "name": "那須清峰高校",
          "slug": "nasuseiho"
        },
        {
          "display": "佐野",
          "score": 9,
          "won": false,
          "name": "佐野高校",
          "slug": "sano"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "大田原",
          "score": 15,
          "won": true,
          "name": "大田原高校",
          "slug": "otawara"
        },
        {
          "display": "真岡工",
          "score": 2,
          "won": false,
          "name": "真岡工業高校",
          "slug": "mokakogyo"
        }
      ]
    },
    {
      "date": "2026-07-10",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "烏山",
          "score": 5,
          "won": true,
          "name": "烏山高校",
          "slug": "karasuyama"
        },
        {
          "display": "上三川",
          "score": 3,
          "won": false,
          "name": "上三川高校",
          "slug": "kaminokawa"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮商",
          "score": 6,
          "won": true,
          "name": "宇都宮商業高校",
          "slug": "utsunomiyashogyo"
        },
        {
          "display": "幸福学園",
          "score": 3,
          "won": false,
          "name": "幸福学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "足利大附",
          "score": 7,
          "won": true,
          "name": "足利大附",
          "slug": null
        },
        {
          "display": "宇都宮中央",
          "score": 1,
          "won": false,
          "name": "宇都宮中央高校",
          "slug": "utsunomiyachuo"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "佐野松桜",
          "score": 5,
          "won": true,
          "name": "佐野松桜高校",
          "slug": "sanosho"
        },
        {
          "display": "連合",
          "score": 4,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "小山西",
          "score": 6,
          "won": false,
          "name": "小山西高校",
          "slug": "oyamanishi"
        },
        {
          "display": "小山",
          "score": 7,
          "won": true,
          "name": "小山高校",
          "slug": "oyama"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "宇都宮",
          "score": 12,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        },
        {
          "display": "矢板東",
          "score": 0,
          "won": false,
          "name": "矢板東高校",
          "slug": "yaitahigashi"
        }
      ]
    },
    {
      "date": "2026-07-11",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "那須拓陽",
          "score": 6,
          "won": true,
          "name": "那須拓陽高校",
          "slug": "nasutakuyo"
        },
        {
          "display": "鹿沼南",
          "score": 4,
          "won": false,
          "name": "鹿沼南高校",
          "slug": "kanumaminami"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "黒磯",
          "score": 2,
          "won": false,
          "name": "黒磯高校",
          "slug": "kuroiso"
        },
        {
          "display": "矢板中央",
          "score": 9,
          "won": true,
          "name": "矢板中央",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮南",
          "score": 2,
          "won": false,
          "name": "宇都宮南高校",
          "slug": "utsunomiyaminami"
        },
        {
          "display": "栃木商",
          "score": 4,
          "won": true,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮北",
          "score": 5,
          "won": true,
          "name": "宇都宮北高校",
          "slug": "utsunomiyakita"
        },
        {
          "display": "茂木",
          "score": 0,
          "won": false,
          "name": "茂木高校",
          "slug": "motegi"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮工",
          "score": 3,
          "won": true,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        },
        {
          "display": "宇都宮白楊",
          "score": 2,
          "won": false,
          "name": "宇都宮白楊高校",
          "slug": "utsunomiyahakuyo"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "連合",
          "score": 3,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "宇都宮東",
          "score": 10,
          "won": true,
          "name": "宇都宮東高校",
          "slug": "utsunomiyahigashi"
        }
      ]
    },
    {
      "date": "2026-07-12",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "小山高専",
          "score": 1,
          "won": false,
          "name": "小山工業高専",
          "slug": "oyamakogyo"
        },
        {
          "display": "連合",
          "score": 4,
          "won": true,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "大田原",
          "score": 0,
          "won": false,
          "name": "大田原高校",
          "slug": "otawara"
        },
        {
          "display": "作新学院",
          "score": 7,
          "won": true,
          "name": "作新学院",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "烏山",
          "score": 1,
          "won": false,
          "name": "烏山高校",
          "slug": "karasuyama"
        },
        {
          "display": "石橋",
          "score": 7,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇短大附",
          "score": 0,
          "won": false,
          "name": "宇短大附",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 10,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "鹿沼",
          "score": 16,
          "won": true,
          "name": "鹿沼高校",
          "slug": "kanuma"
        },
        {
          "display": "鹿沼商工",
          "score": 0,
          "won": false,
          "name": "鹿沼商工高校",
          "slug": "kanumashoko"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "連合",
          "score": 6,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "足利工",
          "score": 7,
          "won": true,
          "name": "足利工業高校",
          "slug": "ashikagakogyo"
        }
      ]
    },
    {
      "date": "2026-07-13",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "高根沢",
          "score": 7,
          "won": true,
          "name": "高根沢高校",
          "slug": "takanezawa"
        },
        {
          "display": "那須清峰",
          "score": 5,
          "won": false,
          "name": "那須清峰高校",
          "slug": "nasuseiho"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "佐野松桜",
          "score": 0,
          "won": false,
          "name": "佐野松桜高校",
          "slug": "sanosho"
        },
        {
          "display": "文星芸大附",
          "score": 14,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "白鷗大足利",
          "score": 10,
          "won": true,
          "name": "白鷗大足利",
          "slug": null
        },
        {
          "display": "宇都宮北",
          "score": 2,
          "won": false,
          "name": "宇都宮北高校",
          "slug": "utsunomiyakita"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "栃木工",
          "score": 2,
          "won": false,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        },
        {
          "display": "宇都宮",
          "score": 3,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "足利大附",
          "score": 2,
          "won": false,
          "name": "足利大附",
          "slug": null
        },
        {
          "display": "栃木",
          "score": 5,
          "won": true,
          "name": "栃木高校",
          "slug": "tochigi"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "宇都宮商",
          "score": 11,
          "won": true,
          "name": "宇都宮商業高校",
          "slug": "utsunomiyashogyo"
        },
        {
          "display": "足利",
          "score": 0,
          "won": false,
          "name": "足利高校",
          "slug": "ashikaga"
        }
      ]
    },
    {
      "date": "2026-07-14",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "小山",
          "score": 0,
          "won": false,
          "name": "小山高校",
          "slug": "oyama"
        },
        {
          "display": "那須拓陽",
          "score": 9,
          "won": true,
          "name": "那須拓陽高校",
          "slug": "nasutakuyo"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "矢板中央",
          "score": 3,
          "won": false,
          "name": "矢板中央",
          "slug": null
        },
        {
          "display": "國學院栃木",
          "score": 9,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮工",
          "score": 4,
          "won": true,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        },
        {
          "display": "青藍泰斗",
          "score": 1,
          "won": false,
          "name": "青藍泰斗",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮東",
          "score": 4,
          "won": true,
          "name": "宇都宮東高校",
          "slug": "utsunomiyahigashi"
        },
        {
          "display": "小山南",
          "score": 3,
          "won": false,
          "name": "小山南高校",
          "slug": "oyamaminami"
        }
      ]
    },
    {
      "date": "2026-07-15",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "栃木商",
          "score": 10,
          "won": true,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        },
        {
          "display": "連合",
          "score": 3,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "高根沢",
          "score": 0,
          "won": false,
          "name": "高根沢高校",
          "slug": "takanezawa"
        },
        {
          "display": "佐野日大",
          "score": 18,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮商",
          "score": 4,
          "won": false,
          "name": "宇都宮商業高校",
          "slug": "utsunomiyashogyo"
        },
        {
          "display": "宇都宮",
          "score": 5,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "作新学院",
          "score": 8,
          "won": true,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "鹿沼",
          "score": 0,
          "won": false,
          "name": "鹿沼高校",
          "slug": "kanuma"
        }
      ]
    },
    {
      "date": "2026-07-18",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "足利工",
          "score": 0,
          "won": false,
          "name": "足利工業高校",
          "slug": "ashikagakogyo"
        },
        {
          "display": "石橋",
          "score": 11,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        }
      ]
    },
    {
      "date": "2026-07-19",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮東",
          "score": 0,
          "won": false,
          "name": "宇都宮東高校",
          "slug": "utsunomiyahigashi"
        },
        {
          "display": "國學院栃木",
          "score": 11,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-19",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮工",
          "score": 7,
          "won": true,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        },
        {
          "display": "栃木商",
          "score": 4,
          "won": false,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        }
      ]
    },
    {
      "date": "2026-07-19",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 4,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "那須拓陽",
          "score": 2,
          "won": false,
          "name": "那須拓陽高校",
          "slug": "nasutakuyo"
        }
      ]
    },
    {
      "date": "2026-07-19",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "白鷗大足利",
          "score": 6,
          "won": true,
          "name": "白鷗大足利",
          "slug": null
        },
        {
          "display": "栃木",
          "score": 1,
          "won": false,
          "name": "栃木高校",
          "slug": "tochigi"
        }
      ]
    },
    {
      "date": "2026-07-22",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "準々決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "作新学院",
          "score": 5,
          "won": true,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "石橋",
          "score": 4,
          "won": false,
          "name": "石橋高校",
          "slug": "ishibashi"
        }
      ]
    },
    {
      "date": "2026-07-22",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "準々決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 11,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "白鷗大足利",
          "score": 4,
          "won": false,
          "name": "白鷗大足利",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-22",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "準々決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "佐野日大",
          "score": 9,
          "won": true,
          "name": "佐野日大",
          "slug": null
        },
        {
          "display": "宇都宮",
          "score": 0,
          "won": false,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2026-07-22",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "準々決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮工",
          "score": 4,
          "won": false,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        },
        {
          "display": "國學院栃木",
          "score": 11,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-24",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "準決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "作新学院",
          "score": 0,
          "won": false,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 5,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2026-07-24",
      "season": "summer",
      "tournament": "第108回全国高等学校野球選手権栃木大会",
      "round": "準決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 2,
          "won": false,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "國學院栃木",
          "score": 5,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "真岡工",
          "score": 0,
          "won": false,
          "name": "真岡工業高校",
          "slug": "mokakogyo"
        },
        {
          "display": "宇都宮",
          "score": 10,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮中央",
          "score": 10,
          "won": true,
          "name": "宇都宮中央高校",
          "slug": "utsunomiyachuo"
        },
        {
          "display": "佐野",
          "score": 1,
          "won": false,
          "name": "佐野高校",
          "slug": "sano"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮商",
          "score": 9,
          "won": true,
          "name": "宇都宮商業高校",
          "slug": "utsunomiyashogyo"
        },
        {
          "display": "鹿沼商工",
          "score": 4,
          "won": false,
          "name": "鹿沼商工高校",
          "slug": "kanumashoko"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "高根沢",
          "score": 11,
          "won": true,
          "name": "高根沢高校",
          "slug": "takanezawa"
        },
        {
          "display": "小山北桜",
          "score": 3,
          "won": false,
          "name": "小山北桜高校",
          "slug": "oyamahokuo"
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "烏山",
          "score": 2,
          "won": false,
          "name": "烏山高校",
          "slug": "karasuyama"
        },
        {
          "display": "連合",
          "score": 5,
          "won": true,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "連合",
          "score": 5,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "幸福学園",
          "score": 10,
          "won": true,
          "name": "幸福学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-13",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "矢板中央",
          "score": 6,
          "won": false,
          "name": "矢板中央",
          "slug": null
        },
        {
          "display": "鹿沼",
          "score": 10,
          "won": true,
          "name": "鹿沼高校",
          "slug": "kanuma"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "栃木商",
          "score": 7,
          "won": true,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        },
        {
          "display": "宇都宮北",
          "score": 2,
          "won": false,
          "name": "宇都宮北高校",
          "slug": "utsunomiyakita"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "上三川",
          "score": 4,
          "won": false,
          "name": "上三川高校",
          "slug": "kaminokawa"
        },
        {
          "display": "大田原",
          "score": 7,
          "won": true,
          "name": "大田原高校",
          "slug": "otawara"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮白楊",
          "score": 3,
          "won": false,
          "name": "宇都宮白楊高校",
          "slug": "utsunomiyahakuyo"
        },
        {
          "display": "足利大附",
          "score": 13,
          "won": true,
          "name": "足利大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "小山",
          "score": 5,
          "won": true,
          "name": "小山高校",
          "slug": "oyama"
        },
        {
          "display": "那須拓陽",
          "score": 2,
          "won": false,
          "name": "那須拓陽高校",
          "slug": "nasutakuyo"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "那須清峰",
          "score": 4,
          "won": false,
          "name": "那須清峰高校",
          "slug": "nasuseiho"
        },
        {
          "display": "さくら清修",
          "score": 5,
          "won": true,
          "name": "さくら清修高校",
          "slug": "sakuraseishu"
        }
      ]
    },
    {
      "date": "2025-09-14",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "真岡ハイトラ運動公園市民球場",
      "teams": [
        {
          "display": "真岡",
          "score": 1,
          "won": false,
          "name": "真岡高校",
          "slug": "moka"
        },
        {
          "display": "白鷗大足利",
          "score": 7,
          "won": true,
          "name": "白鷗大足利",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇短大附",
          "score": 10,
          "won": true,
          "name": "宇短大附",
          "slug": null
        },
        {
          "display": "宇都宮東",
          "score": 1,
          "won": false,
          "name": "宇都宮東高校",
          "slug": "utsunomiyahigashi"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "鹿沼南",
          "score": 14,
          "won": true,
          "name": "鹿沼南高校",
          "slug": "kanumaminami"
        },
        {
          "display": "連合",
          "score": 3,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "茂木",
          "score": 1,
          "won": false,
          "name": "茂木高校",
          "slug": "motegi"
        },
        {
          "display": "宇都宮南",
          "score": 6,
          "won": true,
          "name": "宇都宮南高校",
          "slug": "utsunomiyaminami"
        }
      ]
    },
    {
      "date": "2025-09-15",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "1回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "連合",
          "score": 0,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "栃木",
          "score": 28,
          "won": true,
          "name": "栃木高校",
          "slug": "tochigi"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 9,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "宇都宮商",
          "score": 0,
          "won": false,
          "name": "宇都宮商業高校",
          "slug": "utsunomiyashogyo"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "青藍泰斗",
          "score": 7,
          "won": false,
          "name": "青藍泰斗",
          "slug": null
        },
        {
          "display": "幸福学園",
          "score": 8,
          "won": true,
          "name": "幸福学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "鹿沼",
          "score": 14,
          "won": true,
          "name": "鹿沼高校",
          "slug": "kanuma"
        },
        {
          "display": "足利",
          "score": 2,
          "won": false,
          "name": "足利高校",
          "slug": "ashikaga"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮中央",
          "score": 2,
          "won": false,
          "name": "宇都宮中央高校",
          "slug": "utsunomiyachuo"
        },
        {
          "display": "栃木工",
          "score": 9,
          "won": true,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "宇都宮工",
          "score": 10,
          "won": true,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        },
        {
          "display": "大田原",
          "score": 7,
          "won": false,
          "name": "大田原高校",
          "slug": "otawara"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "黒磯",
          "score": 6,
          "won": true,
          "name": "黒磯高校",
          "slug": "kuroiso"
        },
        {
          "display": "栃木商",
          "score": 2,
          "won": false,
          "name": "栃木商業高校",
          "slug": "tochigishogyo"
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "高根沢",
          "score": 11,
          "won": true,
          "name": "高根沢高校",
          "slug": "takanezawa"
        },
        {
          "display": "連合",
          "score": 7,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        }
      ]
    },
    {
      "date": "2025-09-20",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "足利工",
          "score": 5,
          "won": false,
          "name": "足利工業高校",
          "slug": "ashikagakogyo"
        },
        {
          "display": "宇都宮",
          "score": 12,
          "won": true,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "足利大附",
          "score": 3,
          "won": false,
          "name": "足利大附",
          "slug": null
        },
        {
          "display": "國學院栃木",
          "score": 7,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "宇都宮南",
          "score": 1,
          "won": false,
          "name": "宇都宮南高校",
          "slug": "utsunomiyaminami"
        },
        {
          "display": "佐野日大",
          "score": 8,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "小山高専",
          "score": 5,
          "won": false,
          "name": "小山工業高専",
          "slug": "oyamakogyo"
        },
        {
          "display": "小山",
          "score": 9,
          "won": true,
          "name": "小山高校",
          "slug": "oyama"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "石橋",
          "score": 5,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        },
        {
          "display": "白鷗大足利",
          "score": 2,
          "won": false,
          "name": "白鷗大足利",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "作新学院",
          "score": 23,
          "won": true,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "鹿沼南",
          "score": 0,
          "won": false,
          "name": "鹿沼南高校",
          "slug": "kanumaminami"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "小山西",
          "score": 11,
          "won": true,
          "name": "小山西高校",
          "slug": "oyamanishi"
        },
        {
          "display": "さくら清修",
          "score": 0,
          "won": false,
          "name": "さくら清修高校",
          "slug": "sakuraseishu"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "小山南",
          "score": 1,
          "won": false,
          "name": "小山南高校",
          "slug": "oyamaminami"
        },
        {
          "display": "栃木",
          "score": 11,
          "won": true,
          "name": "栃木高校",
          "slug": "tochigi"
        }
      ]
    },
    {
      "date": "2025-09-21",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "2回戦",
      "venue": "とちぎ木の花スタジアム",
      "teams": [
        {
          "display": "連合",
          "score": 1,
          "won": false,
          "name": "連合",
          "slug": null,
          "combined": true
        },
        {
          "display": "宇短大附",
          "score": 8,
          "won": true,
          "name": "宇短大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-22",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 18,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "高根沢",
          "score": 0,
          "won": false,
          "name": "高根沢高校",
          "slug": "takanezawa"
        }
      ]
    },
    {
      "date": "2025-09-22",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "栃木工",
          "score": 6,
          "won": true,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        },
        {
          "display": "宇都宮",
          "score": 2,
          "won": false,
          "name": "宇都宮高校",
          "slug": "utsunomiya"
        }
      ]
    },
    {
      "date": "2025-09-22",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "幸福学園",
          "score": 10,
          "won": true,
          "name": "幸福学園",
          "slug": null
        },
        {
          "display": "鹿沼",
          "score": 1,
          "won": false,
          "name": "鹿沼高校",
          "slug": "kanuma"
        }
      ]
    },
    {
      "date": "2025-09-22",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "黒磯",
          "score": 5,
          "won": false,
          "name": "黒磯高校",
          "slug": "kuroiso"
        },
        {
          "display": "宇都宮工",
          "score": 8,
          "won": true,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "小山",
          "score": 2,
          "won": false,
          "name": "小山高校",
          "slug": "oyama"
        },
        {
          "display": "國學院栃木",
          "score": 9,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "小山西",
          "score": 0,
          "won": false,
          "name": "小山西高校",
          "slug": "oyamanishi"
        },
        {
          "display": "石橋",
          "score": 2,
          "won": true,
          "name": "石橋高校",
          "slug": "ishibashi"
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "栃木",
          "score": 1,
          "won": false,
          "name": "栃木高校",
          "slug": "tochigi"
        },
        {
          "display": "佐野日大",
          "score": 4,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-23",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "3回戦",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "作新学院",
          "score": 11,
          "won": true,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "宇短大附",
          "score": 1,
          "won": false,
          "name": "宇短大附",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "準々決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "幸福学園",
          "score": 2,
          "won": true,
          "name": "幸福学園",
          "slug": null
        },
        {
          "display": "宇都宮工",
          "score": 1,
          "won": false,
          "name": "宇都宮工業高校",
          "slug": "utsunomiyakogyo"
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "準々決勝",
      "venue": "エイジェックスタジアム",
      "teams": [
        {
          "display": "作新学院",
          "score": 3,
          "won": false,
          "name": "作新学院",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 6,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "準々決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 5,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "栃木工",
          "score": 1,
          "won": false,
          "name": "栃木工業高校",
          "slug": "tochigikogyo"
        }
      ]
    },
    {
      "date": "2025-09-27",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "準々決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "石橋",
          "score": 1,
          "won": false,
          "name": "石橋高校",
          "slug": "ishibashi"
        },
        {
          "display": "國學院栃木",
          "score": 5,
          "won": true,
          "name": "國學院栃木",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-10-04",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "準決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 6,
          "won": true,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "幸福学園",
          "score": 2,
          "won": false,
          "name": "幸福学園",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-10-04",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "準決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "國學院栃木",
          "score": 4,
          "won": false,
          "name": "國學院栃木",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 6,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    },
    {
      "date": "2025-10-05",
      "season": "autumn",
      "tournament": "第78回秋季栃木県高等学校野球大会",
      "round": "決勝",
      "venue": "宇都宮清原球場",
      "teams": [
        {
          "display": "文星芸大附",
          "score": 3,
          "won": false,
          "name": "文星芸大附",
          "slug": null
        },
        {
          "display": "佐野日大",
          "score": 4,
          "won": true,
          "name": "佐野日大",
          "slug": null
        }
      ]
    }
  ]
};
