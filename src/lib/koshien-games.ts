 ...
 * ★**記念大会（第90回・第100回）の「北大阪」「東神奈川」なども県に寄せる**（2026-09-21）——
 *   歴代優勝校の一覧（`koshien-champions.ts`）は県をその年の区分で書いており、
 *   都道府県で数えると 大阪桐蔭2018（北大阪）が「大阪」に入らなかった。
 *   **大会記事から読んだ試合の `pref` には元から入っていない**ので、そちらは1件も変わらない。
 */
