...
export function buildPrefectureLead(input: PrefectureLeadInput): string[] {
  ...
  */
  /*
    ★★★**「0校を掲載しています」と書かない**（2026-09-05 その2）。
    **北海道と東京は「地方大会だけの地区」**で、**学校は1校も紐づかない**
    （学校は北北海道・南北海道…のどれかに属している）。
    ★**0を書かず、学校がどこにあるかを書く。**
    ★**「0勝と書かない」と同じ筋**（数えた結果の0を、あるかのように出さない）。
  */
  paragraphs.push(
    schoolCount > 0
      ? `${prefecture.fullName}の公立高校野球をまとめたページです。` +
          `県立・市立・町村立に加え、国立と高専もあわせた${schoolCount}校を掲載しています。`
      : `${prefecture.fullName}の地方大会をまとめたページです。` +
          `${prefecture.name}の学校の一覧は、甲子園の大会区分ごとのページにあります。`,
  );
