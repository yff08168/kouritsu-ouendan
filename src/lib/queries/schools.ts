...
/*
  ...
  **落ちる枚数は回ごとに変わり**（0枚・7枚・149枚・238枚）、**遅いのは Supabase の待ち**だった。
  ...
  ★★★**1時間に延ばした**（2026-09-09。**Supabase が転送量の上限でプロジェクトごと
  止められた** —— `exceed_egress_quota`。**サイトが丸ごと500になり、ビルドも通らなくなった**）。

  ★★**この索引がいちばん重い読み取り** —— **1回につき schools 3,500行を4リクエスト**で、
  **速報のページを毎回作り直す形にしたぶん、読む回数が増えていた**（2026-09-08）。
  ★**学校マスタは文科省の一覧から作り直すときしか変わらない**ので、
  **5分で追いかける必要がそもそも無い。**
  ★**永久に持たないこと** —— 学校マスタを入れ替えたのに古い索引を返し続ける。
  ★**入れ替えたときは1時間で追いつく**（すぐ反映したいならデプロイし直す）。
*/
const SCHOOL_NAME_ROWS_TTL_MS = 60 * 60 * 1000;
